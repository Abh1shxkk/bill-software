...
const TransactionDateValidator = {
    ...
    fetchDateRange: function (transactionType) {
        ...
        // Get base URL dynamically (handles subdirectory installations)
        const baseUrl = self.getBaseUrl();

        return $.ajax({
            url: baseUrl + '/admin/api/transaction-date-range/' + transactionType,
            method: 'GET',
            dataType: 'json'
    ...
    getBaseUrl: function () {
        ...
        if (match && match[1]) {
            return match[1]; // Returns subdirectory like '/bill-software1'
        }
