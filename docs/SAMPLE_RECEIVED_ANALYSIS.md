# Sample Received Module - Analysis

## Current State

### Transaction Blade (`resources/views/admin/sample-received/transaction.blade.php`)
- **HAS** reusable modal components included
- **Modal IDs**: Generic names
  - Item Modal: `chooseItemsModal`
  - Batch Modal: `batchSelectionModal`
- **Rate Type**: `s_rate` (CORRECT - receiving samples at sale rate for valuation)
- **Show Only Available**: `false` (CORRECT - can receive new batches)
- **Show Cost Details**: `false` (correct - cost not primary concern for samples)
...
### Modification Blade (`resources/views/admin/sample-received/modification.blade.php`)
...
- ❌ Modal IDs need to be descriptive: `sampleReceivedItemModal`, `sampleReceivedBatchModal`
...
- ❌ Modal IDs: `sampleReceivedModItemModal`, `sampleReceivedModBatchModal`
...
### Sample Issued vs Sample Received
...
Sample Received can accept NEW batches that don't exist in the system yet, so we need to allow batch creation/selection even if qty is 0.
...
1. **Transaction Blade**:
   ...
   - Add bridge function `onItemBatchSelectedFromModal()`
   - Add bridge function `showItemSelectionModal()` that calls `window.openItemModal_sampleReceivedItemModal()`
...
2. **Modification Blade**:
   ...
   - Add bridge function `onItemBatchSelectedFromModal()`
   - Add bridge function `showItemSelectionModal()` that calls `window.openItemModal_sampleReceivedModItemModal()`
...
- [ ] Transaction: Select batch from modal (or create new)
...
### Sample Received vs Sample Issued
- **Sample Issued**: Giving samples OUT (decreases stock)
- **Sample Received**: Getting samples IN (increases stock)

### Why Same Rate Type?
Both use `s_rate` because:
- Sample Issued: Values outgoing samples at sale rate
- Sample Received: Values incoming samples at sale rate for consistency
- This maintains consistent valuation across the sample lifecycle

### Batch Handling
- Sample Issued: Can only issue from existing batches (`showOnlyAvailable: true`)
- Sample Received: Can receive into new or existing batches (`showOnlyAvailable: false`)
