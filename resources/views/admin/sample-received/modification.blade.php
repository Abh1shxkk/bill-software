    ...
    if (e.key === 'Enter' || e.key === 'Tab') {
        ...
        if (activeQty && activeQty.name && activeQty.name.includes('[qty]')) {
            const hasModalOpenForQty = document.getElementById('loadInvoiceModal') ||
                document.querySelector('#sampleReceivedModItemModal.show') ||
                document.querySelector('#sampleReceivedModBatchModal.show') ||
...
<script>
// ============================================================
// AUTO-SAVE  —  sample_received_modification_autosave_v1
// ============================================================
(function(){
'use strict';
const KEY = 'sample_received_modification_autosave_v1';
let _t = null;
...
function save(){
    ...
    document.querySelectorAll('#itemsTableBody tr').forEach(function(tr){
        ...
        rows.push({
            ...
            original_batch_id:g('original_batch_id'), original_qty:g('original_qty'),
        });
    });
    ...
    const state={
        ...
        partyTypeInput:_val('partyTypeInput'),
        party_id:_val('party_id'),
        party_name:_val('party_name'),
        partyNameInput:_val('partyNameInput'),
        trn_no:_val('trn_no'),
...
function restore(){
    ...
    _banner(state.savedAt, function keep(){
        ...
        const ptd=document.getElementById('partyTypeInput'); if(ptd) ptd.value=state.partyTypeInput||'';
        _set('party_id',state.party_id||'');
        _set('party_name',state.party_name||'');
        const pnd=document.getElementById('partyNameInput'); if(pnd) pnd.value=state.partyNameInput||'';
        ...
        (state.rows||[]).forEach(function(saved){
            ...
            tr.innerHTML=
                ...
                '<td><input type="text" class="form-control form-control-sm" name="items['+ri+'][expiry]" value="'+_esc(saved.expiry)+'" onkeydown="handleExpiryKeydown(event,'+ri+')" data-custom-enter></td>'+
                '<td><input type="number" class="form-control form-control-sm" name="items['+ri+'][qty]" value="'+_esc(saved.qty||'')+'" onchange="calculateRowAmount('+ri+')" onkeydown="handleQtyKeydown(event,'+ri+')" data-custom-enter></td>'+
                ...
                '<input type="hidden" name="items['+ri+'][original_batch_id]" value="'+_esc(saved.original_batch_id)+'">'+
                '<input type="hidden" name="items['+ri+'][original_qty]" value="'+_esc(saved.original_qty||0)+'">';
