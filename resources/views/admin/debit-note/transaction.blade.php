...
@section('title', 'Debit Note Transaction')
            ...
            <div class="row g-3">
                ...
                <div class="col-md-2">
                    ...
                    <input type="text" class="form-control readonly-field" id="debitNoteNo" 
                           value="{{ $nextDebitNoteNo }}" readonly>
                </div>
                <div class="col-md-3">
...
function updatePartyDropdown() {
    ...
    if (partyType === 'S') {
        ...
        @endforeach
        // Auto-select Purchase in Credit Account Type
        ...
        @endforeach
        // Auto-select Sale in Credit Account Type
...
function fetchSupplierInvoices(supplierId) {
    ...
    Promise.all([
        fetch(`{{ url('admin/debit-note/supplier') }}/${supplierId}/purchase-invoices`)
            .then(r => {
                console.log('Purchase invoices response:', r.status);
                return r.json();
            }),
        fetch(`{{ url('admin/debit-note/supplier') }}/${supplierId}/credit-notes`)
            .then(r => {
    ...
    .then(result => {
        if (result.success) {
            ...
            if (window.printAfterSave) {
                window.open(`{{ url('admin/debit-note') }}/${result.id}/show`, '_blank');
            }
