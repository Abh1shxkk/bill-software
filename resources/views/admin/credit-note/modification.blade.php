...
@media (max-width: 767px) {
    ...
    /* ── Credit Note Details: col-md-* → full width ── */
    .row.g-3 > .col-md-2,
    .row.g-3 > .col-md-3,
    .row.g-3 > .col-md-4,
    .row.g-3 > .col-md-6,
    ...
    /* ── Party Details: Credit + Debit col-md-6 → stack ── */
    .col-md-6 {
        flex: 0 0 100% !important;
        max-width: 100% !important;
    }
...
<script>
// ============================================================
// AUTO-SAVE  —  credit_note_modification_autosave_v1
// ============================================================
(function(){
'use strict';
const KEY = 'credit_note_modification_autosave_v1';
let _t = null;
...
function save(){
    ...
    const state={
        ...
        creditNoteId:_val('creditNoteId'),
        creditNoteDate:_val('creditNoteDate'),
        reason:_val('reason'),         // native select in MOD
        credit_party_type:_radio('credit_party_type'),
        partySelect:_val('partySelect'),
        partySearchInput:_val('partySearchInput'),
        salesmanSelect:_val('salesmanSelect'),
        debit_account_type:_radio('debit_account_type'),
        accountNo:_val('accountNo'),
        invRefNo:_val('invRefNo'),
        invoiceDate:_val('invoiceDate'),
        gstVno:_val('gstVno'),
        partyTrnNo:_val('partyTrnNo'),
        partyTrnDate:_val('partyTrnDate'),
        amount:_val('amount'),
        narration:_val('narration'),
        tcsAmount:_val('tcsAmount'),
        roundOff:_val('roundOff'),
        grossAmount:_val('grossAmount'),
        totalGst:_val('totalGst'),
        netAmount:_val('netAmount'),
        cnAmount:_val('cnAmount'),
        rows:rows,
    };
    if(!state.creditNoteId && !rows.length) return;
...
function restore(){
    ...
    if(!state||!state.creditNoteId) return;
    _banner(state.savedAt, function keep(){
        _set('creditNoteId',state.creditNoteId||'');
        if(state.creditNoteDate) _set('creditNoteDate',state.creditNoteDate);
        if(typeof updateDayName==='function') updateDayName();
        // reason = native select
        _set('reason',state.reason||'');
        if(state.credit_party_type) _setRadio('credit_party_type',state.credit_party_type);
        ...
        if(state.debit_account_type) _setRadio('debit_account_type',state.debit_account_type);
        ...
        _set('cnAmount',state.cnAmount||'0.00');
        ...
        (state.rows||[]).forEach(function(saved){
            const n=typeof hsnRowCount!=='undefined'?hsnRowCount:saved.idx;
            const tr=document.createElement('tr');
            ...
            tr.innerHTML=
                ...
                '<td><button type="button" class="btn btn-danger btn-sm" onclick="removeHsnRow(this)"><i class="bi bi-trash"></i></button></td>';
...
document.addEventListener('DOMContentLoaded',function(){
    setTimeout(function(){
        // Patch markAsSaving → clear draft before save
        const _origMark=(typeof window.markAsSaving==='function')?window.markAsSaving:null;
        window.markAsSaving=function(){
            clearAutoSave();
            if(_origMark) _origMark.apply(this,arguments);
        };
        // Also patch updateCreditNote directly
        if(typeof updateCreditNote==='function'){
            const _origUpdate=updateCreditNote;
            window.updateCreditNote=function(){clearAutoSave();_origUpdate.apply(this,arguments);};
        }
    },800);
