...
<script>
// ============================================================
// AUTO-SAVE  —  stock_transfer_incoming_modification_autosave_v1
// ============================================================
(function(){
'use strict';
const KEY = 'stock_transfer_incoming_modification_autosave_v1';
let _t = null;
...
function save(){
    ...
    const state={
        ...
        transaction_id:_val('transaction_id')||txId,
        currentTransactionId:txId,
        stim_transaction_date:_val('stim_transaction_date'),
        supplier_id:_val('supplier_id'),
        stim_supplierDisplay:_val('stim_supplierDisplay'),
        stim_remarks:_val('stim_remarks'),
        stim_st_date:_val('stim_st_date'),
        stim_gr_no:_val('stim_gr_no'),
        stim_gr_date:_val('stim_gr_date'),
        stim_cases:_val('stim_cases'),
        stim_transport:_val('stim_transport'),
        srlno:_val('srlno'),
        rows:rows,
    };
...
function restore(){
    ...
    _banner(state.savedAt, function keep(){
        ...
        try{ if(state.currentTransactionId) window.currentTransactionId=state.currentTransactionId; }catch(e){}
        if(state.stim_transaction_date) _set('stim_transaction_date',state.stim_transaction_date);
        if(typeof updateDayName==='function') updateDayName();
        _set('supplier_id',state.supplier_id||'');
        const sd=document.getElementById('stim_supplierDisplay'); if(sd) sd.value=state.stim_supplierDisplay||'';
        _set('stim_remarks',state.stim_remarks||'');
        if(state.stim_st_date) _set('stim_st_date',state.stim_st_date);
        _set('stim_gr_no',state.stim_gr_no||'');
        if(state.stim_gr_date) _set('stim_gr_date',state.stim_gr_date);
        _set('stim_cases',state.stim_cases||'');
        _set('stim_transport',state.stim_transport||'');
