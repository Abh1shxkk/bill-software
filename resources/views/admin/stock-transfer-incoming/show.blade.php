...
@section('title', 'View Stock Transfer Incoming')
...
<section class="py-3">
    <div class="container-fluid">
        <div class="d-flex justify-content-between align-items-center mb-3">
            <div>
                <h4 class="mb-0"><i class="bi bi-box-arrow-in-down me-2"></i> Stock Transfer Incoming Details</h4>
                <div class="text-muted small">Transaction #{{ $transaction->trf_no }}</div>
            </div>
            <div>
        ...
        <div class="card shadow-sm border-0 rounded mb-3">
            ...
            <div class="card-body">
                <div class="row">
                    <div class="col-md-3">
                        <p><strong>Trf No.:</strong> {{ $transaction->trf_no }}</p>
                        <p><strong>Date:</strong> {{ $transaction->transaction_date->format('d-M-Y') }}</p>
                        <p><strong>Day:</strong> {{ $transaction->day_name }}</p>
                    </div>
                    <div class="col-md-3">
                        <p><strong>Supplier:</strong> {{ $transaction->supplier ? $transaction->supplier->name : $transaction->supplier_name }}</p>
                        <p><strong>ST Date:</strong> {{ $transaction->st_date ? $transaction->st_date->format('d-M-Y') : '-' }}</p>
                    </div>
                    <div class="col-md-3">
                    ...
                    <table class="table table-bordered table-striped mb-0" style="font-size: 11px;">
                        <thead class="table-success">
                            <tr>
                                ...
                                <th class="text-end">F.T.Rate</th>
                                <th class="text-end">F.T.Amt</th>
                            </tr>
                        </thead>
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td class="text-end">{{ number_format($item->p_rate, 2) }}</td>
                                <td class="text-center">{{ number_format($item->gst_percent, 2) }}%</td>
                                <td class="text-end">{{ number_format($item->ft_rate, 2) }}</td>
                                <td class="text-end">{{ number_format($item->ft_amount, 2) }}</td>
                            </tr>
                            @endforeach
                        </tbody>
