...
@section('title', 'Stock Transfer Incoming Transaction')
                        ...
                        <div class="row g-2 mb-2">
                            ...
                            <div class="col-md-2">
                                <div class="field-group">
                                    ...
                                    <input type="text" id="trf_no" name="trf_no" class="form-control readonly-field" value="{{ $nextTrfNo }}" readonly>
                                </div>
                            </div>
                            <div class="col-md-3">
                                <div class="field-group">
...
function saveTransaction() {
    ...
    fetch('{{ route("admin.stock-transfer-incoming.store") }}', {
        method: 'POST',
        headers: {
...
<script>
// ============================================================
// AUTO-SAVE  —  stock_transfer_incoming_transaction_autosave_v1
// ============================================================
(function(){
'use strict';
const KEY = 'stock_transfer_incoming_transaction_autosave_v1';
let _t = null;
...
function save(){
    ...
    document.querySelectorAll('#itemsTableBody tr').forEach(function(tr){
        ...
        rows.push({
            ...
            qty:g('qty'), free_qty:g('free_qty'), p_rate:g('p_rate'),
            gst_percent:g('gst_percent'), ft_rate:g('ft_rate'), ft_amount:g('ft_amount'),
        });
    });
    const state={
        savedAt:new Date().toISOString(),
        sti_transaction_date:_val('sti_transaction_date'),
        supplier_id:_val('supplier_id'),
        sti_supplierDisplay:_val('sti_supplierDisplay'),
        sti_remarks:_val('sti_remarks'),
        sti_st_date:_val('sti_st_date'),
        sti_gr_no:_val('sti_gr_no'),
        sti_gr_date:_val('sti_gr_date'),
        sti_cases:_val('sti_cases'),
        sti_transport:_val('sti_transport'),
        srlno:_val('srlno'),
        rows:rows,
    };
...
function restore(){
    ...
    _banner(state.savedAt, function keep(){
        if(state.sti_transaction_date) _set('sti_transaction_date',state.sti_transaction_date);
        if(typeof updateDayName==='function') updateDayName();
        _set('supplier_id',state.supplier_id||'');
        const sd=document.getElementById('sti_supplierDisplay'); if(sd) sd.value=state.sti_supplierDisplay||'';
        _set('sti_remarks',state.sti_remarks||'');
        if(state.sti_st_date) _set('sti_st_date',state.sti_st_date);
        _set('sti_gr_no',state.sti_gr_no||'');
        if(state.sti_gr_date) _set('sti_gr_date',state.sti_gr_date);
        _set('sti_cases',state.sti_cases||'');
        _set('sti_transport',state.sti_transport||'');
        ...
        (state.rows||[]).forEach(function(saved){
            ...
            tr.innerHTML=
                ...
                '<td><input type="text" class="form-control form-control-sm" name="items['+ri+'][expiry]" value="'+_esc(saved.expiry)+'" placeholder="MM/YY" onkeydown="handleExpiryKeydown(event,'+ri+')" onfocus="selectRow('+ri+')" data-custom-enter></td>'+
                ...
                '<td><input type="number" class="form-control form-control-sm" name="items['+ri+'][free_qty]" value="'+_esc(saved.free_qty||0)+'" step="1" min="0" onkeydown="handleFreeQtyKeydown(event,'+ri+')" onfocus="selectRow('+ri+')" data-custom-enter></td>'+
                '<td><input type="number" class="form-control form-control-sm" name="items['+ri+'][p_rate]" value="'+parseFloat(saved.p_rate||0).toFixed(2)+'" step="0.01" onchange="calculateRowAmount('+ri+')" onkeydown="handlePRateKeydown(event,'+ri+')" onfocus="selectRow('+ri+')" data-custom-enter></td>'+
                '<td><input type="number" class="form-control form-control-sm" name="items['+ri+'][gst_percent]" value="'+_esc(saved.gst_percent||0)+'" step="0.01" min="0" onchange="calculateRowAmount('+ri+')" onkeydown="handleGstKeydown(event,'+ri+')" onfocus="selectRow('+ri+')" data-custom-enter></td>'+
                '<td><input type="number" class="form-control form-control-sm readonly-field" name="items['+ri+'][ft_rate]" value="'+_esc(saved.ft_rate||'')+'" step="0.01" readonly onfocus="selectRow('+ri+')"></td>'+
                '<td><input type="number" class="form-control form-control-sm readonly-field" name="items['+ri+'][ft_amount]" value="'+_esc(saved.ft_amount||'')+'" step="0.01" readonly onfocus="selectRow('+ri+')"></td>'+
...
document.addEventListener('DOMContentLoaded',function(){
    ...
    const form=document.getElementById('stiForm');
