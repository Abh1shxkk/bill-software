...
@section('title', 'Claim to Supplier')
@section('content')
...
<div class="card shadow-sm border-0 rounded">
    <div class="card mb-4">
        <div class="card-body">
            <form method="GET" action="{{ route('admin.claim-to-supplier.index') }}" class="row g-3" id="filterForm">
                <div class="col-md-3">
                    <label for="filter_by" class="form-label">Filter By</label>
                    <select class="form-select" id="filter_by" name="filter_by">
                        <option value="claim_no" {{ request('filter_by', 'claim_no') == 'claim_no' ? 'selected' : '' }}>Claim No.</option>
        ...
        <table class="table table-hover align-middle mb-0">
            ...
            <tbody id="cts-table-body">
                ...
                <tr>
                    ...
                    <td class="text-end">
                        <a class="btn btn-sm btn-outline-info" href="{{ route('admin.claim-to-supplier.show', $transaction->id) }}" title="View Details">
                            <i class="bi bi-eye"></i>
                        </a>
                        <a class="btn btn-sm btn-outline-primary" href="{{ route('admin.claim-to-supplier.modification') }}?claim_no={{ $transaction->claim_no }}" title="Edit">
                            <i class="bi bi-pencil"></i>
                        </a>
                        <button type="button" class="btn btn-sm btn-outline-danger delete-cts" 
                                data-id="{{ $transaction->id }}" 
                                data-claim-no="{{ $transaction->claim_no }}" 
                                data-supplier="{{ $transaction->supplier_name ?? 'Unknown' }}"
                                title="Delete">
    ...
    <div class="card-footer bg-light d-flex flex-column gap-2">
        ...
        <div id="cts-sentinel" data-next-url="{{ $transactions->appends(request()->query())->nextPageUrl() }}" style="height: 1px;"></div>
        @endif
    </div>
    @endif
</div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function deleteTransaction(id, modal) {
        ...
        fetch(`{{ url('admin/claim-to-supplier') }}/${id}`, {
            method: 'DELETE',
            headers: {
