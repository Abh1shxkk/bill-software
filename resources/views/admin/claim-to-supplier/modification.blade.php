...
function populateFormWithData(header, items) {
    ...
    // Set supplier - handle Select2 if initialized
    if ($('#supplier_id').hasClass('select2-hidden-accessible')) {
        $('#supplier_id').val(header.supplier_id).trigger('change');
    } else {
    ...
    // Select first row and update calculation/summary sections (with delay for DOM)
    setTimeout(function() {
        if (items.length > 0) {
