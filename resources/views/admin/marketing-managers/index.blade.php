            ...
            <div class="d-flex justify-content-between align-items-center mb-4 flex-wrap gap-2">
                <div class="d-flex align-items-center gap-3 flex-wrap">
                    ...
                    @include('layouts.partials.module-shortcuts', [
                        'createRoute' => route('admin.marketing-managers.create'),
                        'tableBodyId' => 'marketingManagersTableBody',
                        'checkboxClass' => 'marketing-managers-checkbox'
                    ])
                </div>
                <div class="d-flex gap-2">
                    <button type="button" id="delete-selected-marketing-managers-btn" class="btn btn-danger d-none" onclick="confirmMultipleDeleteMarketingManagers()">
                        <i class="bi bi-trash me-2"></i>Delete Selected (<span id="selected-marketing-managers-count">0</span>)
                    </button>
                </div>
            </div>
                            ...
                            <tr>
                                <th>
                                    <div class="form-check">
                                        <input class="form-check-input" type="checkbox" id="select-all-marketing-managers">
                                        <label class="form-check-label" for="select-all-marketing-managers">
                        ...
                        <tbody id="marketingManagersTableBody">
                            @forelse($marketingManagers as $marketingManager)
                                <tr>
                                    <td>
                                        <div class="form-check">
                                            <input class="form-check-input marketing-managers-checkbox" type="checkbox" value="{{ $marketingManager->id }}" id="marketing-manager-{{ $marketingManager->id }}">
                                            <label class="form-check-label" for="marketing-manager-{{ $marketingManager->id }}">
                                                <span class="visually-hidden">Select marketing manager</span>
                                            </label>
                                        </div>
                                    </td>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function performSearch() {
        ...
        .then(html => {
            ...
            if (typeof window.reattachMarketingManagersEventListeners === 'function') {
                window.reattachMarketingManagersEventListeners();
                window.updateMarketingManagersSelectedCount && window.updateMarketingManagersSelectedCount();
            }
        })
        .catch(error => {
    ...
    window.performMarketingManagersSearch = performSearch;

    // Multiple delete selection management for marketing managers
    let marketingManagersPageElements = {
        selectAllCheckbox: document.getElementById('select-all-marketing-managers'),
        deleteSelectedBtn: document.getElementById('delete-selected-marketing-managers-btn'),
        selectedCountSpan: document.getElementById('selected-marketing-managers-count')
    };

    window.updateMarketingManagersSelectedCount = function() {
        const checkedBoxes = document.querySelectorAll('.marketing-managers-checkbox:checked');
        const count = checkedBoxes.length;
        if (marketingManagersPageElements.selectedCountSpan) {
            marketingManagersPageElements.selectedCountSpan.textContent = count;
        }
        if (marketingManagersPageElements.deleteSelectedBtn) {
            if (count > 0) marketingManagersPageElements.deleteSelectedBtn.classList.remove('d-none');
            else marketingManagersPageElements.deleteSelectedBtn.classList.add('d-none');
        }
        if (marketingManagersPageElements.selectAllCheckbox) {
            const allBoxes = document.querySelectorAll('.marketing-managers-checkbox');
            if (count === 0) {
                marketingManagersPageElements.selectAllCheckbox.indeterminate = false;
                marketingManagersPageElements.selectAllCheckbox.checked = false;
            } else if (count === allBoxes.length) {
                marketingManagersPageElements.selectAllCheckbox.indeterminate = false;
                marketingManagersPageElements.selectAllCheckbox.checked = true;
            } else {
                marketingManagersPageElements.selectAllCheckbox.indeterminate = true;
                marketingManagersPageElements.selectAllCheckbox.checked = false;
            }
        }
    };

    window.reattachMarketingManagersEventListeners = function() {
        setTimeout(() => {
            document.querySelectorAll('.marketing-managers-checkbox').forEach(cb => {
                cb.removeEventListener('change', window.updateMarketingManagersSelectedCount);
                cb.addEventListener('change', function(){
                    window.updateMarketingManagersSelectedCount();
                });
            });
            const selectAll = document.getElementById('select-all-marketing-managers');
            if (selectAll) {
                const newSelectAll = selectAll.cloneNode(true);
                selectAll.parentNode.replaceChild(newSelectAll, selectAll);
                newSelectAll.addEventListener('change', function(){
                    const boxes = document.querySelectorAll('.marketing-managers-checkbox');
                    boxes.forEach(b => b.checked = this.checked);
                    window.updateMarketingManagersSelectedCount();
                });
                marketingManagersPageElements.selectAllCheckbox = newSelectAll;
            }
        }, 50);
    };
...
// Confirm multiple delete for Marketing Managers
function confirmMultipleDeleteMarketingManagers() {
    const checked = document.querySelectorAll('.marketing-managers-checkbox:checked');
    ...
    window.GlobalMultipleDelete.show({
        ...
        deleteUrl: '{{ route('admin.marketing-managers.multiple-delete') }}',
        itemType: 'marketing managers',
        csrfToken: document.querySelector('meta[name="csrf-token"]').getAttribute('content'),
        onSuccess: function(){ window.performMarketingManagersSearch(); },
        onError: function(err){
            console.error('Error deleting marketing managers:', err);
            if (window.crudNotification) {
                crudNotification.showToast('error', 'Error', 'Failed to delete selected marketing managers. Please try again.');
            }
        }
    });
}
...
function viewMarketingManagerDetails(id) {
  ...
  const url = `{{ url('/admin/marketing-managers') }}/${id}`;
  fetch(url, {
    headers: {
