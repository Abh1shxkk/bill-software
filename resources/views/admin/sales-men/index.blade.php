      ...
      <tbody id="salesmen-table-body">
        @forelse($salesMen as $salesMan)
          <tr>
            <td>
              <div class="form-check">
                <input class="form-check-input sales-men-checkbox" type="checkbox" value="{{ $salesMan->id }}" id="sales-men-{{ $salesMan->id }}">
                <label class="form-check-label" for="sales-men-{{ $salesMan->id }}">
...
function viewSalesManDetails(salesManId) {
  ...
  const url = `{{ url('/admin/sales-men') }}/${salesManId}`;
  fetch(url, {
    method: 'GET',
    headers: {
      'X-Requested-With': 'XMLHttpRequest',
...
function confirmMultipleDeleteSalesMen() {
  ...
  window.GlobalMultipleDelete.show({
    ...
    deleteUrl: '{{ route('admin.sales-men.multiple-delete') }}',
    itemType: 'sales-men',
