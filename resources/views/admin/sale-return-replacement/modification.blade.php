...
$(document).ready(function() {
    ...
    $('#btnLoad').click(function() {
        let trnNo = $('#loadTrnNo').val();
        if(!trnNo) return alert('Enter S.R.No');
        ...
        $.ajax({
                ...
                if(res.success) {
                    ...
                    if(d.items && d.items.length > 0) {
                        d.items.forEach((item, i) => {
                            let newRow = `<tr>
                                <td><input type="text" class="form-control item-code" name="items[${i}][item_code]" value="${item.item_code || ''}"></td>
                                ...
                                <td><input type="number" step="any" class="form-control qty" name="items[${i}][qty]" value="${item.qty || 0}"></td>
                                <td><input type="number" step="any" class="form-control f-qty" name="items[${i}][free_qty]" value="${item.free_qty || 0}"></td>
                                <td><input type="number" step="any" class="form-control sale-rate" name="items[${i}][sale_rate]" value="${item.sale_rate || 0}"></td>
                                ...
                                <td><input type="number" step="any" class="form-control ft-rate" name="items[${i}][ft_rate]" value="${item.ft_rate || 0}"></td>
                                <td><input type="number" step="any" class="form-control amount" name="items[${i}][amount]" value="${item.amount || 0}" readonly></td>
                                <td class="text-center"><button type="button" class="btn btn-danger btn-sm remove-row"><i class="bi bi-x"></i></button></td>
                            </tr>`;
            ...
            error: function() { alert('Transaction not found'); }
        });
    });
