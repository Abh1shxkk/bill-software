...
document.addEventListener('keydown', function(e) {
    if (e.key === 'Enter' || e.key === 'Tab') {
        ...
        if (activeQty && activeQty.name && activeQty.name.includes('[qty]')) {
            const hasModalOpenForQty = document.getElementById('itemModal')?.classList.contains('show') ||
...
<script>
// ============================================================
// AUTO-SAVE  —  stock_transfer_incoming_return_transaction_autosave_v1
// ============================================================
(function(){
'use strict';
const KEY = 'stock_transfer_incoming_return_transaction_autosave_v1';
let _t = null;
...
function save(){
    ...
    document.querySelectorAll('#itemsTableBody tr').forEach(function(tr){
        ...
        rows.push({
            ...
            batchData:tr.dataset.batchData||'{}',
    ...
    const state={
        ...
        stirt_transaction_date:_val('stirt_transaction_date'),
        supplier_id:_val('supplier_id'),
        name:_val('name'),
        stirt_supplierDisplay:_val('stirt_supplierDisplay'),
        stirt_remarks:_val('stirt_remarks'),
        stirt_gr_no:_val('stirt_gr_no'),
        stirt_gr_date:_val('stirt_gr_date'),
        stirt_cases:_val('stirt_cases'),
        stirt_transport:_val('stirt_transport'),
        srlno:_val('srlno'),
        rows:rows,
    };
...
function restore(){
    ...
    _banner(state.savedAt, function keep(){
        if(state.stirt_transaction_date) _set('stirt_transaction_date',state.stirt_transaction_date);
        if(typeof updateDayName==='function') updateDayName();
        _set('supplier_id',state.supplier_id||'');
        _set('name',state.name||'');
        const sd=document.getElementById('stirt_supplierDisplay'); if(sd) sd.value=state.stirt_supplierDisplay||'';
        _set('stirt_remarks',state.stirt_remarks||'');
        _set('stirt_gr_no',state.stirt_gr_no||'');
        if(state.stirt_gr_date) _set('stirt_gr_date',state.stirt_gr_date);
        _set('stirt_cases',state.stirt_cases||'');
        _set('stirt_transport',state.stirt_transport||'');
        ...
        (state.rows||[]).forEach(function(saved){
            ...
            tr.dataset.batchData=saved.batchData||'{}';
