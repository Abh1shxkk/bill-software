...
<div class="d-flex justify-content-between align-items-center mb-3">
  <div>
    ...
    <div class="text-muted small">Complete details of purchase transaction</div>
  </div>
  <div>
    <a href="{{ route('admin.purchase.invoices') }}" class="btn btn-outline-secondary">
      <i class="bi bi-arrow-left"></i> Back to Invoices
    </a>
    <a href="{{ route('admin.purchase.modification', ['trn_no' => $transaction->trn_no]) }}" class="btn btn-primary">
      ...
      <div class="card-body">
        <div class="row g-3">
          <div class="col-md-6">
            ...
            <div class="form-control-plaintext">{{ $transaction->trn_no }}</div>
          </div>
          <div class="col-md-6">
            <label class="form-label fw-bold">Bill No:</label>
            <div class="form-control-plaintext">{{ $transaction->bill_no ?? 'N/A' }}</div>
          </div>
          <div class="col-md-6">
            <label class="form-label fw-bold">Bill Date:</label>
            <div class="form-control-plaintext">{{ $transaction->bill_date ? $transaction->bill_date->format('d/m/Y') : 'N/A' }}</div>
          </div>
          <div class="col-md-6">
            <label class="form-label fw-bold">Receive Date:</label>
            <div class="form-control-plaintext">{{ $transaction->receive_date ? $transaction->receive_date->format('d/m/Y') : 'N/A' }}</div>
          </div>
          <div class="col-md-6">
  ...
  <div class="col-md-4">
    <div class="card shadow-sm border-0 rounded">
      <div class="card-header bg-info text-white">
        <h5 class="mb-0"><i class="bi bi-building me-2"></i>Supplier Details</h5>
      </div>
      <div class="card-body">
        <div class="mb-3">
          <label class="form-label fw-bold">Supplier Name:</label>
          <div class="form-control-plaintext">{{ $transaction->supplier->name ?? 'N/A' }}</div>
        </div>
        <div class="mb-3">
          <label class="form-label fw-bold">Supplier ID:</label>
          <div class="form-control-plaintext">{{ $transaction->supplier_id ?? 'N/A' }}</div>
        </div>
        <div class="mb-3">
    ...
    <div class="card shadow-sm border-0 rounded">
      ...
      <div class="card-body">
        <div class="row g-2">
          ...
          <div class="col-6">
            <label class="form-label fw-bold">Less Amount:</label>
            <div class="form-control-plaintext">₹{{ number_format($transaction->less_amount ?? 0, 2) }}</div>
          </div>
          <div class="col-6">
    ...
    <div class="card shadow-sm border-0 rounded">
      ...
      <div class="card-body">
        <div class="row g-3">
          ...
          <div class="col-6">
            <label class="form-label fw-bold">TOF Amount:</label>
            <div class="form-control-plaintext">₹{{ number_format($transaction->tof_amount ?? 0, 2) }}</div>
          </div>
          <div class="col-12">
            <hr>
            <label class="form-label fw-bold fs-5">Final Invoice Amount:</label>
            <div class="form-control-plaintext fs-4 fw-bold text-success">
              ₹{{ number_format($transaction->inv_amount ?? 0, 2) }}
            </div>
          </div>
        </div>
      </div>
