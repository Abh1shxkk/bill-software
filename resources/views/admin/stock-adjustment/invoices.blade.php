...
@section('title', 'Stock Adjustment Invoices')
...
<div class="d-flex justify-content-between align-items-center mb-3">
    ...
    <div class="d-flex gap-2">
        ...
        <a href="{{ route('admin.stock-adjustment.modification') }}" class="btn btn-warning">
...
<div class="card shadow-sm border-0 rounded">
            ...
            <tbody id="adjustmentsTableBody">
                ...
                <tr>
                    ...
                    <td class="text-center">
                        <span class="badge bg-info text-white">{{ $adjustment->total_items }}</span>
                    </td>
                    <td class="text-center">
                        <span class="badge bg-danger">{{ $adjustment->shortage_items }}</span>
                    </td>
                    <td class="text-center">
                        <span class="badge bg-success">{{ $adjustment->excess_items }}</span>
                    </td>
                    <td class="text-end fw-bold {{ $adjustment->total_amount < 0 ? 'text-danger' : 'text-success' }}">
                        ₹{{ number_format(abs($adjustment->total_amount), 2) }}
                    </td>
                    <td>{{ Str::limit($adjustment->remarks, 30) ?? '-' }}</td>
                    <td class="text-end">
                        <a href="{{ route('admin.stock-adjustment.modification', ['trn_no' => $adjustment->trn_no]) }}" class="btn btn-sm btn-outline-secondary" title="Edit">
    ...
    <div class="card-footer bg-light d-flex flex-column gap-2">
        <div class="d-flex justify-content-between align-items-center w-100">
            <div>Showing {{ $adjustments->firstItem() ?? 0 }}-{{ $adjustments->lastItem() ?? 0 }} of {{ $adjustments->total() }}</div>
            <div class="text-muted">Page {{ $adjustments->currentPage() }} of {{ $adjustments->lastPage() }}</div>
        </div>
        ...
        <div id="adjustment-sentinel" data-next-url="{{ $adjustments->appends(request()->query())->nextPageUrl() }}" style="height: 1px;"></div>
        @endif
    </div>
</div>
...
window.performAdjustmentSearch = function() {
    ...
    fetch(`{{ route('admin.stock-adjustment.invoices') }}?${params.toString()}`, {
        headers: { 'X-Requested-With': 'XMLHttpRequest' }
    })
...
function confirmDelete() {
    ...
    fetch(`{{ url('admin/stock-adjustment') }}/${deleteId}`, {
        method: 'DELETE',
        headers: {
