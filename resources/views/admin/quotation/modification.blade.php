...
/* Table qty/rate inputs — delegate to inline handlers with qty validation */
window.addEventListener('keydown', function(e) {
    if (e.key !== 'Enter') return;
    if (_qtAnyModalOpen()) return;
    ...
    // Check if qty field and validate qty > 0
    if (active.name?.includes('[qty]')) {
...
function handleQtyKeydown(event, rowIndex) {
    ...
    // Block navigation if qty is 0 or empty
    if (qty <= 0) {
        event.preventDefault();
