...
<script>
// ============================================================
// AUTO-SAVE ON REFRESH  —  bsi_issued_modification_autosave_v1
// ============================================================
(function () {
    'use strict';
    const STORAGE_KEY = 'bsi_issued_modification_autosave_v1';
    let _timer = null;
    ...
    function saveDraft() {
        ...
        document.querySelectorAll('#itemsTableBody tr').forEach(function(row) {
            ...
            const gs = function(n) { const el = row.querySelector('select[name*="['+n+']"]'); return el ? el.value : ''; };
            rows.push({
                id: gh('id'),
    ...
    function _restoreRow(saved) {
        const tbody = document.getElementById('itemsTableBody');
        const idx = rowIndex++;
        const brVal = saved.br_ex || 'B';
        const tr = document.createElement('tr');
        tr.id = 'row_' + idx;
        tr.onclick = function() { selectRow(idx); };
        tr.innerHTML =
            '<td><input type="text" name="items['+idx+'][item_code]" value="'+_esc(saved.item_code)+'" readonly class="readonly-field"></td>' +
            '<td><input type="text" name="items['+idx+'][item_name]" value="'+_esc(saved.item_name)+'" readonly class="readonly-field"></td>' +
            '<td><input type="text" name="items['+idx+'][batch_no]" value="'+_esc(saved.batch_no)+'" readonly class="readonly-field"></td>' +
            '<td><input type="text" name="items['+idx+'][expiry]" value="'+_esc(saved.expiry)+'" readonly class="readonly-field"></td>' +
            '<td><input type="number" name="items['+idx+'][qty]" value="'+_esc(saved.qty)+'" min="0" class="text-end" onchange="calculateRowAmount('+idx+')"></td>' +
            '<td><input type="number" name="items['+idx+'][free_qty]" value="'+_esc(saved.free_qty||0)+'" min="0" class="text-end"></td>' +
            '<td><input type="number" name="items['+idx+'][rate]" value="'+parseFloat(saved.rate||0).toFixed(2)+'" step="0.01" class="text-end" onchange="calculateRowAmount('+idx+')"></td>' +
            '<td><input type="number" name="items['+idx+'][dis_percent]" value="'+_esc(saved.dis_percent||0)+'" step="0.01" class="text-end" onchange="calculateRowAmount('+idx+')"></td>' +
            '<td><input type="number" name="items['+idx+'][scm_percent]" value="'+_esc(saved.scm_percent||0)+'" step="0.01" class="text-end" onchange="calculateRowAmount('+idx+')"></td>' +
            '<td><select name="items['+idx+'][br_ex]" class="form-control br-ex-select no-select2" id="br_ex_'+idx+'"><option value="B"'+(brVal==='B'?' selected':'')+'>Brk</option><option value="E"'+(brVal==='E'?' selected':'')+'>Exp</option></select></td>' +
    ...
    function restoreDraft() {
        ...
        if (!state || (!state.transaction_id && !(state.rows && state.rows.length))) return;
        ...
        _banner(state.savedAt, function keep() {
            ...
            if (typeof updateSupplierName === 'function') updateSupplierName();
    ...
    document.addEventListener('DOMContentLoaded', function() {
        setTimeout(function() {
            ...
            window.fetch = function(url) {
                ...
                if (s.indexOf('breakage-supplier') !== -1 && s.indexOf('issued') !== -1) {
