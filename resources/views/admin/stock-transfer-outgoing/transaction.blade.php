...
<style>
    ...
    .modal-header-custom { padding: 1rem; background: #0d6efd; color: white; display: flex; justify-content: space-between; align-items: center; border-radius: 8px 8px 0 0; }
...
<div class="d-flex justify-content-between align-items-center mb-3">
    <div>
        <h4 class="mb-0 d-flex align-items-center"><i class="bi bi-box-arrow-right me-2"></i> Stock Transfer Outgoing - Transaction</h4>
        ...
        <form id="stockTransferOutgoingForm" method="POST" autocomplete="off">
            ...
            <div class="header-section">
                <div class="row g-2">
                    ...
                    <div class="col-md-6">
                        <div class="field-group">
                            ...
                            <div class="custom-dropdown" id="customerDropdownWrapper" style="flex: 1; position: relative;">
                                <input type="text" class="form-control form-control-sm" id="customerDisplay" 
                    ...
                    <div class="col-md-2">
                        <div class="field-group">
                            <label>Remarks:</label>
                            <input type="text" class="form-control form-control-sm" id="sto_remarks" name="remarks" placeholder="Remarks" data-custom-enter>
                        </div>
                    </div>
                    <div class="col-md-2">
                        <div class="field-group">
            ...
            <div class="gr-section">
                <div class="row g-2 align-items-center">
                    <div class="col-auto">
                        <div class="field-group">
                            <label>GR No.:</label>
                            <input type="text" class="form-control form-control-sm" id="sto_gr_no" name="gr_no" style="width: 100px;" data-custom-enter>
                        </div>
                    </div>
                    <div class="col-auto">
                        <div class="field-group">
                    ...
                    <div class="col-auto">
                        <div class="field-group">
                            <label>Cases:</label>
                            <input type="number" class="form-control form-control-sm" id="sto_cases" name="cases" style="width: 70px;" value="0" data-custom-enter>
                        </div>
                    </div>
                    <div class="col-auto">
                        <div class="field-group">
                            <label>Transport:</label>
                            <input type="text" class="form-control form-control-sm" id="sto_transport" name="transport" style="width: 200px;" data-custom-enter>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white border rounded p-2 mb-2">
                ...
                <div class="text-center mt-2">
                    <button type="button" class="btn btn-sm btn-info" id="insertItemsBtn" onclick="openInsertItemsModal()">
...
function selectCustomer(id, name, code) {
    ...
    document.getElementById('customerDisplay').value = code + ' - ' + name;
...
function highlightCustomerItem() {
    ...
    // Wrap around only when navigating with arrow keys (index >= 0)
...
document.addEventListener('keydown', function(e) {
    ...
    if (e.key === 'Enter') {
        ...
        // Skip modal handling (managed separately)
        var itemModal = document.getElementById('stockTransferOutgoingItemModal');
        var batchModal = document.getElementById('stockTransferOutgoingBatchModal');
        ...
        // Backward navigation
        if (e.shiftKey) {
            if (activeEl.id === 'customerDisplay') {
                e.preventDefault();
                document.querySelector('input[name="transaction_date"]')?.focus();
                return false;
            }
            if (activeEl.name === 'remarks') {
                e.preventDefault();
                document.getElementById('customerDisplay')?.focus();
                return false;
            }
            if (activeEl.name === 'gr_no') {
                e.preventDefault();
                document.querySelector('input[name="remarks"]')?.focus();
                return false;
            }
            if (activeEl.name === 'gr_date') {
                e.preventDefault();
                document.querySelector('input[name="gr_no"]')?.focus();
                return false;
            }
            if (activeEl.name === 'cases') {
                e.preventDefault();
                document.querySelector('input[name="gr_date"]')?.focus();
                return false;
            }
            if (activeEl.name === 'transport') {
                e.preventDefault();
                document.querySelector('input[name="cases"]')?.focus();
                return false;
            }
            if (activeEl.name && activeEl.name.includes('[qty]')) {
                e.preventDefault();
            ...
            if (activeEl.name && activeEl.name.includes('[rate]')) {
                e.preventDefault();
                const row = activeEl.closest('tr');
                if (row) {
        ...
        if (activeEl.id === 'customerDisplay') {
            ...
            if (listContainer && listContainer.style.display === 'block') {
                ...
                if (customerActiveIndex >= 0 && customerActiveIndex < items.length) {
                    // User has highlighted an item with arrow keys, select it
                    items[customerActiveIndex].click();
                } else {
                    // No item highlighted, close dropdown and move to next field
        ...
        // Forward navigation
        if (activeEl.name === 'transaction_date') {
            e.preventDefault();
            e.stopPropagation();
            e.stopImmediatePropagation();
        ...
        if (activeEl.name === 'cases') {
            ...
            e.stopPropagation();
            e.stopImmediatePropagation();
            document.querySelector('input[name="transport"]')?.focus();
            return false;
        }
        if (activeEl.name === 'transport') {
            e.preventDefault();
            e.stopPropagation();
            e.stopImmediatePropagation();
            // Trigger Insert Items button click
            const insertBtn = document.getElementById('insertItemsBtn');
            if (insertBtn) {
                insertBtn.focus();
                insertBtn.click();
            } else if (typeof openInsertItemsModal === 'function') {
                openInsertItemsModal();
            }
            return false;
        }
        if (activeEl.id === 'insertItemsBtn') {
            e.preventDefault();
            e.stopPropagation();
            e.stopImmediatePropagation();
        ...
        if (activeEl.name && activeEl.name.includes('[qty]')) {
            ...
            if (row) {
                ...
                if (rowIndex !== undefined) calculateRowAmount(parseInt(rowIndex));
    ...
    if (document.activeElement && document.activeElement.id === 'customerDisplay') {
        const listContainer = document.getElementById('customerList');
        if (listContainer && listContainer.style.display === 'block') {
            if (e.key === 'ArrowDown') {
                e.preventDefault();
...
// We can leave this empty since selectCustomer sets transfer_to_name
...
function addNewRow() {
    ...
    row.innerHTML = `
        ...
        <td><input type="number" class="form-control text-end" name="items[${rowIndex}][qty]" value="0" min="0" onchange="calculateRowAmount(${rowIndex})" data-custom-enter></td>
        <td><input type="number" class="form-control text-end" name="items[${rowIndex}][rate]" value="0" step="0.01" onchange="calculateRowAmount(${rowIndex})" data-custom-enter></td>
...
function addItemToTableWithoutBatch(item) {
    ...
    row.innerHTML = `
        ...
        <td><input type="text" class="form-control" name="items[${rowIndex}][expiry]" readonly></td>
        <td><input type="number" class="form-control text-end" name="items[${rowIndex}][qty]" value="0" min="0" onchange="calculateRowAmount(${rowIndex})" data-custom-enter></td>
        <td><input type="number" class="form-control text-end" name="items[${rowIndex}][rate]" value="${parseFloat(item.s_rate || item.mrp || 0).toFixed(2)}" step="0.01" onchange="calculateRowAmount(${rowIndex})" data-custom-enter></td>
        <td><input type="number" class="form-control text-end readonly-field" name="items[${rowIndex}][amount]" value="0.00" readonly></td>
        <td class="text-center"><button type="button" class="btn btn-danger btn-sm" onclick="removeRow(${rowIndex})"><i class="bi bi-trash"></i></button></td>
    `;
...
function addItemToTable(item, batch) {
    ...
    row.innerHTML = `
        ...
        <td><input type="number" class="form-control text-end" name="items[${rowIndex}][qty]" value="0" min="0" onchange="calculateRowAmount(${rowIndex})" data-custom-enter></td>
        <td><input type="number" class="form-control text-end" name="items[${rowIndex}][rate]" value="${rate}" step="0.01" onchange="calculateRowAmount(${rowIndex})" data-custom-enter></td>
        <td><input type="number" class="form-control text-end readonly-field" name="items[${rowIndex}][amount]" value="0.00" readonly></td>
        <td class="text-center"><button type="button" class="btn btn-danger btn-sm" onclick="removeRow(${rowIndex})"><i class="bi bi-trash"></i></button></td>
    `;
...
<script>
// ============================================================
// AUTO-SAVE  —  stock_transfer_outgoing_transaction_autosave_v1
// ============================================================
(function(){
'use strict';
const KEY = 'stock_transfer_outgoing_transaction_autosave_v1';
let _t = null;
...
function save(){
    ...
    const state={
        ...
        sto_transaction_date:_val('sto_transaction_date'),
        customerSelect:_val('customerSelect'),
        customerDisplay:_val('customerDisplay'),
        transfer_to_name:_val('transfer_to_name'),
        sto_remarks:_val('sto_remarks'),
        sto_gr_no:_val('sto_gr_no'),
        sto_gr_date:_val('sto_gr_date'),
        sto_cases:_val('sto_cases'),
        sto_transport:_val('sto_transport'),
        rows:rows,
    };
    if(!state.customerSelect && !rows.length) return;
...
function restore(){
    ...
    if(!state||(!state.customerSelect&&!state.rows.length)) return;
    _banner(state.savedAt, function keep(){
        if(state.sto_transaction_date) _set('sto_transaction_date',state.sto_transaction_date);
        if(typeof updateDayName==='function') updateDayName();
        _set('customerSelect',state.customerSelect||'');
        const cd=document.getElementById('customerDisplay'); if(cd) cd.value=state.customerDisplay||'';
        _set('transfer_to_name',state.transfer_to_name||'');
        _set('sto_remarks',state.sto_remarks||'');
        _set('sto_gr_no',state.sto_gr_no||'');
        if(state.sto_gr_date) _set('sto_gr_date',state.sto_gr_date);
        _set('sto_cases',state.sto_cases||'0');
        _set('sto_transport',state.sto_transport||'');
        ...
        (state.rows||[]).forEach(function(saved){
            ...
            tr.innerHTML=
                '<td><input type="text" class="form-control" name="items['+ri+'][code]" value="'+_esc(saved.code)+'" onchange="searchItemByCode('+ri+',this.value)" onkeydown="handleCodeKeydown(event,'+ri+')"></td>'+
                '<td><input type="text" class="form-control" name="items['+ri+'][name]" value="'+_esc(saved.name)+'" readonly></td>'+
                '<td><input type="text" class="form-control" name="items['+ri+'][batch]" value="'+_esc(saved.batch)+'" onkeydown="handleBatchKeydown(event,'+ri+')"></td>'+
                '<td><input type="text" class="form-control" name="items['+ri+'][expiry]" value="'+_esc(saved.expiry)+'" readonly></td>'+
                '<td><input type="number" class="form-control text-end" name="items['+ri+'][qty]" value="'+_esc(saved.qty||0)+'" min="0" onchange="calculateRowAmount('+ri+')" data-custom-enter></td>'+
                '<td><input type="number" class="form-control text-end" name="items['+ri+'][rate]" value="'+parseFloat(saved.rate||0).toFixed(2)+'" step="0.01" onchange="calculateRowAmount('+ri+')" data-custom-enter></td>'+
                '<td><input type="number" class="form-control text-end readonly-field" name="items['+ri+'][amount]" value="'+_esc(saved.amount||'0.00')+'" readonly></td>'+
                '<td class="text-center"><button type="button" class="btn btn-danger btn-sm" onclick="removeRow('+ri+')"><i class="bi bi-trash"></i></button></td>';
