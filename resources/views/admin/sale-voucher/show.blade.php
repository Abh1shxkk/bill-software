...
@section('title', 'Sale Voucher Details')
@section('content')
<style>
    .detail-card { background: #fff; border-radius: 8px; box-shadow: 0 2px 8px rgba(0,0,0,0.1); }
    .detail-header { background: linear-gradient(135deg, #38a169, #276749); color: white; padding: 15px 20px; border-radius: 8px 8px 0 0; }
    .detail-body { padding: 20px; }
    .info-grid { display: grid; grid-template-columns: repeat(4, 1fr); gap: 15px; margin-bottom: 20px; }
    .info-item { background: #f8f9fa; padding: 12px; border-radius: 6px; border-left: 3px solid #38a169; }
    ...
    .items-table th { background: #38a169; color: white; padding: 10px; text-align: center; }
    .items-table td { padding: 8px 10px; border-bottom: 1px solid #eee; }
    .items-table tr:hover { background: #f5f5f5; }
    .totals-box { background: #f0fff4; padding: 15px; border-radius: 6px; border: 1px solid #9ae6b4; }
</style>

<div class="d-flex justify-content-between align-items-center mb-3">
    <h5 class="mb-0"><i class="bi bi-eye me-2"></i>Sale Voucher Details</h5>
    <div class="d-flex gap-2">
        <a href="{{ route('admin.sale-voucher.modification') }}?invoice_no={{ $voucher->invoice_no }}" class="btn btn-warning btn-sm">
...
<div class="detail-card">
    <div class="detail-header">
        <div class="d-flex justify-content-between align-items-center">
            <h6 class="mb-0"><i class="bi bi-receipt me-2"></i>Invoice #{{ $voucher->invoice_no }}</h6>
            <span class="badge bg-light text-dark">{{ $voucher->sale_date ? $voucher->sale_date->format('d/m/Y') : '' }}</span>
        </div>
    </div>
    <div class="detail-body">
        <div class="info-grid">
            <div class="info-item">
                ...
                <div class="info-value">{{ $voucher->invoice_no }}</div>
            </div>
            <div class="info-item">
                <div class="info-label">Sale Date</div>
                <div class="info-value">{{ $voucher->sale_date ? $voucher->sale_date->format('d/m/Y') : '-' }}</div>
            </div>
            <div class="info-item">
                <div class="info-label">Customer</div>
                <div class="info-value">{{ $voucher->customer?->name ?? '-' }}</div>
            </div>
            <div class="info-item">
                <div class="info-label">Salesman</div>
                <div class="info-value">{{ $voucher->salesman?->name ?? '-' }}</div>
            </div>
        </div>
        ...
        <table class="items-table mb-4">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td class="text-end fw-bold">₹{{ number_format($item->net_amount ?? ($item->amount + $item->cgst_amount + $item->sgst_amount), 2) }}</td>
                </tr>
                @empty
            ...
            <div class="col-md-6 offset-md-6">
                <div class="totals-box">
                    ...
                    <div class="d-flex justify-content-between">
                        ...
                        <strong class="fs-5 text-success">₹{{ number_format($voucher->net_amount ?? 0, 2) }}</strong>
                    </div>
                </div>
            </div>
        </div>
