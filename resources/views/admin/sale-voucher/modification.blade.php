...
<script>
// ============================================================
// AUTO-SAVE  —  sale_voucher_modification_autosave_v1
// ============================================================
(function(){
'use strict';
const KEY = 'sale_voucher_modification_autosave_v1';
let _t = null;
...
function save(){
    ...
    const state={
        ...
        saleDate:_val('saleDate'), dueDate:_val('dueDate'),
        customerId:_val('customerId'), customerDisplay:_val('svm_customerDisplay'),
        salesmanId:_val('salesmanId'), salesmanDisplay:_val('svm_salesmanDisplay'),
        cashFlag:_val('cashFlag'), remarks:_val('remarks'),
        rows:rows,
    };
...
function restore(){
    ...
    _banner(state.savedAt, function keep(){
        ...
        if(state.saleDate) _set('saleDate',state.saleDate);
        if(state.dueDate) _set('dueDate',state.dueDate);
        _set('customerId',state.customerId||'');
        _set('svm_customerDisplay',state.customerDisplay||'');
        _set('salesmanId',state.salesmanId||'');
        _set('svm_salesmanDisplay',state.salesmanDisplay||'');
        _set('cashFlag',state.cashFlag||'N');
        ...
        (state.rows||[]).forEach(function(saved){
            ...
            tr.innerHTML=
                '<td><input type="text" class="hsn-code" data-row="'+ri+'" data-custom-enter onclick="selectRow('+ri+')" onkeydown="handleHsnKeydown(event,'+ri+')" placeholder="Enter HSN"></td>'+
                '<td><input type="number" class="amount" data-row="'+ri+'" data-custom-enter step="0.01" onchange="calculateRowTax('+ri+')" oninput="calculateRowTax('+ri+')" onkeydown="handleAmountKeydown(event,'+ri+')" placeholder="0.00"></td>'+
                '<td><input type="number" class="gst-percent" data-row="'+ri+'" data-custom-enter step="0.01" onchange="calculateRowTax('+ri+')" onkeydown="handleGstKeydown(event,'+ri+')" placeholder="0"></td>'+
                '<td><input type="number" class="cgst-percent" step="0.01" readonly style="background:#e9ecef;"></td>'+
                '<td><input type="number" class="cgst-amount" step="0.01" readonly style="background:#e9ecef;"></td>'+
                '<td><input type="number" class="sgst-percent" step="0.01" readonly style="background:#e9ecef;"></td>'+
                '<td><input type="number" class="sgst-amount" step="0.01" readonly style="background:#e9ecef;"></td>'+
                '<td><input type="number" class="qty" data-row="'+ri+'" data-custom-enter value="0" step="1" min="0" onkeydown="handleQtyKeydown(event,'+ri+')" placeholder="0"></td>'+
                '<td class="text-center"><button type="button" class="btn btn-sm btn-outline-danger" onclick="deleteRow('+ri+')" title="Delete Row"><i class="bi bi-trash"></i></button></td>';
