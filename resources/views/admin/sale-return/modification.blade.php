...
function populateItemsTable(items) {
    ...
    // Focus on the first row's quantity field (for modification page, focus first row)
    // Then check next row - if empty, focus on its code field
    setTimeout(() => {
        ...
        if (firstRow) {
            ...
            if (qtyInput && !qtyInput.readOnly && !qtyInput.disabled) {
                qtyInput.focus();
                qtyInput.select();
                console.log('Focus set on first row qty input:', qtyInput);
                
                // Get the row index from the row ID
                const rowId = firstRow.id;
                const rowIndex = parseInt(rowId.replace('row-', ''));
                
                // Select the first row for calculation
                selectRowForCalculation(rowIndex);
                
                // Check next row (second row) - if it has no item, focus on its code field
                setTimeout(() => {
                    const secondRow = allRows[1]; // Get the second row
                    if (secondRow) {
                        const codeInput = secondRow.querySelector('input[name*="[code]"]');
                        const itemIdInput = secondRow.querySelector('input[name*="[item_id]"]');
                        const hasItem = itemIdInput && itemIdInput.value && itemIdInput.value !== '';
                        
                        if (!hasItem && codeInput) {
                            // Second row is empty, focus on its code field
                            codeInput.focus();
                            if (codeInput.select) codeInput.select();
                            console.log('Second row is empty, focus set on code field:', codeInput);
                        }
                    }
                }, 200);
            } else {
    ...
    row.innerHTML = `
        ...
        <td>
            <input type="number" class="form-control" name="items[${rowIndex}][free_qty]" value="${item.return_fqty || 0}" step="1"
                   onchange="calculateRowAmount(${rowIndex})"
                   onkeydown="if(event.ctrlKey && (event.key === 'l' || event.key === 'L')) { event.preventDefault(); openSchemeModal(${rowIndex}); return false; } if(event.key === 'Enter') { event.preventDefault(); moveToNextField(${rowIndex}, 'sale_rate'); return false; }"
                   onfocus="selectRowForCalculation(${rowIndex})">
        </td>
        <td>
...
function handleDiscountAndCompleteRow(rowIndex) {
    ...
    // After completing row, check NEXT ROW specifically (not any empty row)
    const allRows = document.querySelectorAll('#itemsTableBody tr');
    let currentRowIndexInTable = -1;
    
    // Find current row index in the table
    for (let i = 0; i < allRows.length; i++) {
        if (allRows[i].id === `row-${rowIndex}`) {
            currentRowIndexInTable = i;
            break;
        }
    }
    
    // Check if there's a next row
    const nextRow = currentRowIndexInTable >= 0 ? allRows[currentRowIndexInTable + 1] : null;
    
    if (nextRow) {
        const nextItemIdInput = nextRow.querySelector('input[name*="[item_id]"]');
        const nextCodeInput = nextRow.querySelector('input[name*="[code]"]');
        const nextQtyInput = nextRow.querySelector('input[name*="[qty]"]');
        const hasItem = nextItemIdInput && nextItemIdInput.value && nextItemIdInput.value !== '';
        
        if (hasItem && nextQtyInput) {
            // Next row has item - go to its QTY column
            console.log('Next row has item, focusing on QTY:', nextRow.id);
            setTimeout(() => {
                nextQtyInput.focus();
                if (nextQtyInput.select) nextQtyInput.select();
                // Select this row for calculation
                const nextRowId = nextRow.id;
                const nextRowIdx = parseInt(nextRowId.replace('row-', ''));
                selectRowForCalculation(nextRowIdx);
            }, 100);
        } else if (!hasItem && nextCodeInput) {
            // Next row is empty - go to its code field
            console.log('Next row is empty, focusing on code:', nextRow.id);
            setTimeout(() => {
                nextCodeInput.focus();
                if (nextCodeInput.select) nextCodeInput.select();
            }, 100);
        }
    } else {
        // No next row - create new one
        console.log('No next row, creating new row');
        addNewRow();
    }
}
