...
<script>
// ============================================================
// AUTO-SAVE  —  replacement_received_modification_autosave_v1
// ============================================================
(function(){
'use strict';
const KEY = 'replacement_received_modification_autosave_v1';
let _t = null;
...
function save(){
    ...
    const state={
        ...
        rr_no:_val('rr_no'), lctn:_val('lctn'),
...
function restore(){
    ...
    _banner(state.savedAt, function keep(){
        ...
        (state.rows||[]).forEach(function(saved){
            ...
            tr.innerHTML=
                '<td><input type="text" class="form-control" name="items['+ri+'][code]" value="'+_esc(saved.code)+'" readonly></td>'+
                '<td><input type="text" class="form-control" name="items['+ri+'][name]" value="'+_esc(saved.name)+'" readonly></td>'+
                '<td><input type="text" class="form-control" name="items['+ri+'][batch]" value="'+_esc(saved.batch)+'" readonly></td>'+
                '<td><input type="text" class="form-control" name="items['+ri+'][expiry]" value="'+_esc(saved.expiry)+'" readonly></td>'+
                '<td><input type="number" class="form-control text-end" name="items['+ri+'][qty]" step="1" min="1" value="'+_esc(saved.qty||0)+'" onchange="calculateRowAmount('+ri+')" onkeydown="handleQtyKeydown(event,'+ri+')"></td>'+
                '<td><input type="number" class="form-control text-end" name="items['+ri+'][free_qty]" value="'+_esc(saved.free_qty||0)+'" step="1" min="0" onkeydown="handleFreeQtyKeydown(event,'+ri+')"></td>'+
                '<td><input type="number" class="form-control text-end" name="items['+ri+'][mrp]" value="'+parseFloat(saved.mrp||0).toFixed(2)+'" step="0.01" onchange="calculateRowAmount('+ri+')" onkeydown="handleMrpKeydown(event,'+ri+')"></td>'+
                '<td><input type="number" class="form-control text-end" name="items['+ri+'][discount_percent]" value="'+_esc(saved.discount_percent||0)+'" step="0.01" min="0" max="100" onchange="calculateRowAmount('+ri+')" onkeydown="handleDiscountKeydown(event,'+ri+')"></td>'+
                '<td><input type="number" class="form-control text-end readonly-field" name="items['+ri+'][ft_rate]" value="'+_esc(saved.ft_rate||0)+'" step="0.01" readonly></td>'+
                '<td><input type="number" class="form-control text-end readonly-field" name="items['+ri+'][ft_amount]" value="'+_esc(saved.ft_amount||0)+'" step="0.01" readonly></td>'+
                '<td><button type="button" class="btn btn-sm btn-danger py-0" onclick="removeRow('+ri+')"><i class="bi bi-x"></i></button></td>';
