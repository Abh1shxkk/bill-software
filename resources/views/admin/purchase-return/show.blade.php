...
<div class="d-flex justify-content-between align-items-center mb-3">
    <div>
        <h4 class="mb-0 d-flex align-items-center">
            <i class="bi bi-arrow-return-left me-2"></i> Purchase Return Transaction Details
        </h4>
        <div class="text-muted small">Complete details of purchase return transaction</div>
    </div>
    <div>
        <a href="{{ route('admin.purchase-return.index') }}" class="btn btn-outline-secondary">
            <i class="bi bi-arrow-left"></i> Back to Invoices
        </a>
        <a href="{{ route('admin.purchase-return.modification') }}?pr_no={{ $transaction->pr_no }}" class="btn btn-outline-warning">
    ...
    <div class="col-md-8">
        <div class="card shadow-sm">
            ...
            <div class="card-body">
                <div class="row g-3">
                    <div class="col-md-6">
                        <label class="fw-bold">PR No:</label>
                        <div>{{ $transaction->pr_no }}</div>
                    </div>
                    <div class="col-md-6">
                        <label class="fw-bold">Return Date:</label>
                        <div>{{ $transaction->return_date ? $transaction->return_date->format('d/m/Y') : '-' }}</div>
                    </div>
                    <div class="col-md-6">
        ...
        <div class="table-responsive">
            <table class="table table-hover mb-0">
                ...
                <tbody>
                    ...
                    @empty
                    <tr>
                        <td colspan="13" class="text-center text-muted py-3">No items found</td>
                    </tr>
                    @endforelse
                </tbody>
            </table>
...
<div class="card shadow-sm">
    <div class="card-header bg-warning text-dark">
        <h5 class="mb-0"><i class="bi bi-credit-card me-2"></i>Credit Note Adjustments</h5>
    </div>
    <div class="card-body p-0">
        <div class="table-responsive">
            <table class="table table-hover mb-0">
                ...
                <tbody>
                    ...
                    <tr>
                        <td>{{ $loop->iteration }}</td>
                        <td>
                            @if($adjustment->purchaseTransaction)
                                {{ $adjustment->purchaseTransaction->bill_no ?? '-' }}
                            @else
                                -
                            @endif
                        </td>
                        <td>
                            @if($adjustment->purchaseTransaction && $adjustment->purchaseTransaction->bill_date)
                                {{ $adjustment->purchaseTransaction->bill_date->format('d/m/Y') }}
                            @else
                                -
                            @endif
                        </td>
