...
<script>
// ============================================================
// AUTO-SAVE  —  sample_issued_transaction_autosave_v1
// ============================================================
(function(){
'use strict';
const KEY = 'sample_issued_transaction_autosave_v1';
let _t = null;
...
function save(){
    ...
    const state={
        ...
        sit_transaction_date:_val('sit_transaction_date'),
        sit_party_type:_val('sit_party_type'),
        sit_partyTypeDisplay:_val('sit_partyTypeDisplay'),
        party_id:_val('party_id'),
        party_name:_val('party_name'),
        sit_partyDisplay:_val('sit_partyDisplay'),
        sit_remarks:_val('sit_remarks'),
        sit_on_field:_val('sit_on_field'),
        sit_rate:_val('sit_rate'),
        sit_tag:_val('sit_tag'),
        sit_gr_no:_val('sit_gr_no'),
        sit_gr_date:_val('sit_gr_date'),
        sit_cases:_val('sit_cases'),
        sit_road_permit:_val('sit_road_permit'),
        sit_truck_no:_val('sit_truck_no'),
        sit_transport:_val('sit_transport'),
        rows:rows,
    };
...
function restore(){
    ...
    _banner(state.savedAt, function keep(){
        if(state.sit_transaction_date) _set('sit_transaction_date',state.sit_transaction_date);
        if(typeof updateDayName==='function') updateDayName();
        _set('sit_party_type',state.sit_party_type||'');
        const ptd=document.getElementById('sit_partyTypeDisplay'); if(ptd) ptd.value=state.sit_partyTypeDisplay||'';
        _set('party_id',state.party_id||'');
        _set('party_name',state.party_name||'');
        const pd=document.getElementById('sit_partyDisplay'); if(pd) pd.value=state.sit_partyDisplay||'';
        _set('sit_remarks',state.sit_remarks||'');
        _set('sit_on_field',state.sit_on_field||'');
        _set('sit_rate',state.sit_rate||'0');
        _set('sit_tag',state.sit_tag||'');
        _set('sit_gr_no',state.sit_gr_no||'');
        if(state.sit_gr_date) _set('sit_gr_date',state.sit_gr_date);
        _set('sit_cases',state.sit_cases||'0');
        _set('sit_road_permit',state.sit_road_permit||'');
        _set('sit_truck_no',state.sit_truck_no||'');
        _set('sit_transport',state.sit_transport||'');
