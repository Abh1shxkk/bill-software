    ...
    if (e.key === 'Enter' || e.key === 'Tab') {
        ...
        if (activeQty && activeQty.name && activeQty.name.includes('[qty]')) {
            const hasModalOpenForQty = document.getElementById('itemModal') || document.getElementById('batchModal') ||
                ...
                document.getElementById('loadInvoiceModal');
...
<script>
// ============================================================
// AUTO-SAVE  —  sample_issued_modification_autosave_v1
// ============================================================
(function(){
'use strict';
const KEY = 'sample_issued_modification_autosave_v1';
let _t = null;
...
function save(){
    ...
    const state={
        ...
        sim_transaction_date:_val('sim_transaction_date'),
        sim_party_type:_val('sim_party_type'),
        sim_partyTypeDisplay:_val('sim_partyTypeDisplay'),
        party_id:_val('party_id'),
        party_name:_val('party_name'),
        sim_partyDisplay:_val('sim_partyDisplay'),
        trn_no:_val('trn_no'),
        sim_remarks:_val('sim_remarks'),
        sim_on_field:_val('sim_on_field'),
        sim_rate:_val('sim_rate'),
        sim_tag:_val('sim_tag'),
        sim_gr_no:_val('sim_gr_no'),
        sim_gr_date:_val('sim_gr_date'),
        sim_cases:_val('sim_cases'),
        sim_road_permit:_val('sim_road_permit'),
        sim_truck_no:_val('sim_truck_no'),
        sim_transport:_val('sim_transport'),
        rows:rows,
    };
...
function restore(){
    ...
    _banner(state.savedAt, function keep(){
        ...
        if(state.sim_transaction_date) _set('sim_transaction_date',state.sim_transaction_date);
        if(typeof updateDayName==='function') updateDayName();
        _set('sim_party_type',state.sim_party_type||'');
        const ptd=document.getElementById('sim_partyTypeDisplay'); if(ptd) ptd.value=state.sim_partyTypeDisplay||'';
        _set('party_id',state.party_id||'');
        _set('party_name',state.party_name||'');
        const pd=document.getElementById('sim_partyDisplay'); if(pd) pd.value=state.sim_partyDisplay||'';
        _set('sim_remarks',state.sim_remarks||'');
        _set('sim_on_field',state.sim_on_field||'');
        _set('sim_rate',state.sim_rate||'0');
        _set('sim_tag',state.sim_tag||'');
        _set('sim_gr_no',state.sim_gr_no||'');
        if(state.sim_gr_date) _set('sim_gr_date',state.sim_gr_date);
        _set('sim_cases',state.sim_cases||'0');
        _set('sim_road_permit',state.sim_road_permit||'');
        _set('sim_truck_no',state.sim_truck_no||'');
        _set('sim_transport',state.sim_transport||'');
        ...
        (state.rows||[]).forEach(function(saved){
            ...
            tr.innerHTML=
                ...
                '<td><input type="number" class="form-control form-control-sm" name="items['+ri+'][qty]" value="'+_esc(saved.qty||'')+'" onchange="calculateRowAmount('+ri+')" onkeydown="handleQtyKeydown(event,'+ri+')" onfocus="selectRow('+ri+')" data-custom-enter></td>'+
                ...
                '<td><button type="button" class="btn btn-sm btn-danger" onclick="removeRow('+ri+')"><i class="bi bi-x"></i></button>'+
                ...
                '<input type="hidden" name="items['+ri+'][mrp]" value="'+_esc(saved.mrp||0)+'"></td>';
