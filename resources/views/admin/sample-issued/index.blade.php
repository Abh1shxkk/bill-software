...
@section('title', 'Sample Issued')
@section('content')

<div class="d-flex justify-content-between align-items-center mb-3">
  <div>
    <h4 class="mb-0 d-flex align-items-center"><i class="bi bi-box-arrow-up me-2"></i> Sample Issued</h4>
    <div class="text-muted small">Manage sample issued transactions</div>
  </div>
  <div>
    <a href="{{ route('admin.sample-issued.create') }}" class="btn btn-primary btn-sm">
...
<div class="card shadow-sm border-0 rounded">
  <div class="card mb-4">
    <div class="card-body">
      <form method="GET" action="{{ route('admin.sample-issued.index') }}" class="row g-3" id="filterForm">
        <div class="col-md-3">
          ...
          <select class="form-select" id="filter_by" name="filter_by">
            ...
            <option value="party_name" {{ request('filter_by') == 'party_name' ? 'selected' : '' }}>Party Name</option>
            <option value="party_type" {{ request('filter_by') == 'party_type' ? 'selected' : '' }}>Party Type</option>
          </select>
        </div>
        <div class="col-md-5">
  ...
  <!-- Table Section -->
  <div class="table-responsive" id="si-table-wrapper" style="position: relative; min-height: 400px;">
      ...
      <tbody id="si-table-body">
        @forelse($transactions ?? [] as $transaction)
          <tr>
            ...
            <td>
              <span class="badge bg-info">{{ $transaction->party_type ?? '-' }}</span>
            </td>
            <td>{{ $transaction->party_name ?? '-' }}</td>
            <td class="text-end">{{ number_format($transaction->total_qty ?? 0, 0) }}</td>
            <td class="text-end">
              <span class="badge bg-success">₹{{ number_format($transaction->net_amount ?? 0, 2) }}</span>
            </td>
            <td class="text-end">
              <a class="btn btn-sm btn-outline-info" href="{{ route('admin.sample-issued.show', $transaction->id) }}" title="View Details">
                <i class="bi bi-eye"></i>
              </a>
              <a class="btn btn-sm btn-outline-primary" href="{{ route('admin.sample-issued.modification') }}?load={{ $transaction->id }}" title="Edit">
                <i class="bi bi-pencil"></i>
              </a>
              <button type="button" class="btn btn-sm btn-outline-danger delete-si" 
                      data-id="{{ $transaction->id }}" 
                      data-trn-no="{{ $transaction->trn_no }}" 
                      data-party="{{ $transaction->party_name ?? 'Unknown' }}"
                      title="Cancel">
        ...
        @empty
          <tr><td colspan="8" class="text-center text-muted">No sample issued transactions found</td></tr>
        @endforelse
      </tbody>
    </table>
  </div>
  ...
  <div class="card-footer bg-light d-flex flex-column gap-2">
    ...
    @if($transactions->hasMorePages())
      <div class="d-flex align-items-center justify-content-center gap-2">
        <div id="si-spinner" class="spinner-border text-primary d-none" style="width: 2rem; height: 2rem;" role="status">
          <span class="visually-hidden">Loading...</span>
        </div>
        <span id="si-load-text" class="text-muted" style="font-size: 0.9rem;">Scroll for more</span>
      </div>
      <div id="si-sentinel" data-next-url="{{ $transactions->appends(request()->query())->nextPageUrl() }}" style="height: 1px;"></div>
    @endif
  </div>
</div>

<!-- Delete Confirmation Modal -->
<div class="modal fade" id="deleteSiModal" tabindex="-1" aria-labelledby="deleteSiModalLabel" aria-hidden="true">
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="deleteSiModalLabel">Confirm Cancel</h5>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>
      <div class="modal-body">
        <p>Are you sure you want to cancel this sample issued transaction?</p>
...
document.addEventListener('DOMContentLoaded', function(){
  ...
  const tableBody = document.getElementById('si-table-body');
  const tableWrapper = document.getElementById('si-table-wrapper');
  ...
  async function fetchData(urlOrParams, pushState = true) {
    try {
      ...
      const newTableBody = doc.querySelector('#si-table-body');
  ...
  document.addEventListener('click', function(e) {
    const button = e.target.closest('.delete-si');
    if (!button) return;
    ...
    const modal = new bootstrap.Modal(document.getElementById('deleteSiModal'));
    modal.show();
  ...
  function initInfiniteScroll() {
    ...
    const sentinel = document.getElementById('si-sentinel');
    const spinner = document.getElementById('si-spinner');
    const loadText = document.getElementById('si-load-text');
    ...
    async function loadMore() {
      ...
      try {
        ...
        const parser = new DOMParser();
        const doc = parser.parseFromString(html, 'text/html');
        const newRows = doc.querySelectorAll('#si-table-body tr');
        ...
        const newSentinel = doc.querySelector('#si-sentinel');
        if (newSentinel) {
