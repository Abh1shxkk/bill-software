...
@section('title', 'Order Form Three Column')

@section('content')
<div class="container-fluid">
    <div class="card mb-2" style="background-color: #ffc4d0;">
        <div class="card-body py-2 text-center">
            <h4 class="mb-0 text-primary fst-italic fw-bold" style="font-family: 'Times New Roman', serif;">Order Form Three Column</h4>
        </div>
    </div>
    ...
    <div class="card shadow-sm" style="background-color: #f0f0f0;">
        <div class="card-body p-3">
            <form method="GET" id="filterForm" action="">
                ...
                <!-- With R(ate)/B(alance)/O(rder Qty) -->
                <div class="row g-2 mb-2 align-items-center">
                    <div class="col-md-4">
                        <label class="fw-bold mb-0">With <u>R</u>(ate)/<u>B</u>(alance)/<u>O</u>(rder Qty) :</label>
                    </div>
                    <div class="col-auto">
                        <select name="rbo_type" id="rbo_type" class="form-select form-select-sm" style="width: 60px;">
                            <option value="R" {{ ($rboType ?? 'R') == 'R' ? 'selected' : '' }}>R</option>
                            <option value="B" {{ ($rboType ?? '') == 'B' ? 'selected' : '' }}>B</option>
                            <option value="O" {{ ($rboType ?? '') == 'O' ? 'selected' : '' }}>O</option>
                        </select>
                    </div>
                </div>

                <!-- N(ormal)/W(holesale)/S(pecial) Rates -->
                <div class="row g-2 mb-2 align-items-center">
                    <div class="col-md-4">
                        <label class="fw-bold mb-0"><u>N</u>(ormal)/<u>W</u>(holesale)/<u>S</u>(pecial) Rates :</label>
                    </div>
                    <div class="col-auto">
                        <select name="nws_rates" id="nws_rates" class="form-select form-select-sm" style="width: 60px;">
                            <option value="N" {{ ($nwsRates ?? 'N') == 'N' ? 'selected' : '' }}>N</option>
                            <option value="W" {{ ($nwsRates ?? '') == 'W' ? 'selected' : '' }}>W</option>
                            <option value="S" {{ ($nwsRates ?? '') == 'S' ? 'selected' : '' }}>S</option>
                        </select>
                    </div>
                </div>
                ...
                <div class="row g-2 mb-2 align-items-center">
                    <div class="col-md-4">
                        <label class="fw-bold mb-0">Full Tax Rate [ Y / N ] :</label>
                    </div>
                    <div class="col-auto">
                        <select name="full_tax_rate" id="full_tax_rate" class="form-select form-select-sm" style="width: 60px;">
                            <option value="Y" {{ ($fullTaxRate ?? 'Y') == 'Y' ? 'selected' : '' }}>Y</option>
                            <option value="N" {{ ($fullTaxRate ?? '') == 'N' ? 'selected' : '' }}>N</option>
                        </select>
                    </div>
                </div>
