...
@section('title', 'Vat-Wise Items')

@section('content')
<div class="container-fluid">
    <div class="card mb-2" style="background-color: #ffc4d0;">
        <div class="card-body py-2 text-center">
            <h4 class="mb-0 text-primary fst-italic fw-bold" style="font-family: serif; letter-spacing: 1px;">Vat-Wise Items</h4>
        </div>
    </div>

    <div class="card shadow-sm" style="background-color: #f0f0f0;">
        <div class="card-body p-3">
            <form method="GET" id="filterForm" action="{{ route('admin.reports.inventory.item.vat-wise') }}">
                <!-- VAT % -->
                <div class="row g-2 mb-2 align-items-center">
                    <div class="col-md-2 text-end pe-2">
                        <label class="fw-bold mb-0">Vat % :</label>
                    </div>
                    <div class="col-md-2">
                        <input type="number" name="vat_percent" id="vat_percent" class="form-control form-control-sm" value="{{ request('vat_percent') }}" step="0.01" placeholder="Enter VAT %">
                    </div>
                </div>
    ...
    <div class="card mt-3">
        <div class="card-header bg-primary text-white py-2 d-flex justify-content-between">
            <strong>Vat-Wise Items Report {{ request('vat_percent') ? '(' . request('vat_percent') . '%)' : '' }}</strong>
            <div>
                ...
                <table class="table table-bordered table-sm table-striped mb-0">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td class="text-end fw-bold">{{ number_format($row['vat_percent'], 2) }}%</td>
                        </tr>
                        @endforeach
                    </tbody>
                </table>
...
function exportToExcel() {
    ...
    window.location.href = '{{ route("admin.reports.inventory.item.vat-wise") }}?' + params.toString();
}

function printReport() {
    const params = new URLSearchParams(new FormData(document.getElementById('filterForm')));
    params.set('print', '1');
    window.open('{{ route("admin.reports.inventory.item.vat-wise") }}?' + params.toString(), 'PrintReport', 'width=900,height=700');
}
