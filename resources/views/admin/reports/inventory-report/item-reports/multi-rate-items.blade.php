...
@section('title', 'Multi Rate Items')

@section('content')
<div class="container-fluid">
    <div class="card mb-2" style="background-color: #ffc4d0;">
        <div class="card-body py-2 text-center">
            <h4 class="mb-0 text-primary fst-italic fw-bold" style="font-family: serif; letter-spacing: 1px;">Multi Rate Items</h4>
        </div>
    </div>

    <div class="card shadow-sm" style="background-color: #f0f0f0;">
        <div class="card-body p-3">
            <form method="GET" id="filterForm" action="{{ route('admin.reports.inventory.item.multi-rate') }}">
                <!-- Date Range -->
                ...
                <div class="row g-2 mb-2 align-items-center">
                    <div class="col-md-2 text-end pe-2">
                        <label class="fw-bold mb-0">Multi Rate :</label>
                    </div>
                    <div class="col-auto">
                        <div class="form-check">
                            <input class="form-check-input" type="checkbox" name="multi_rate" id="multi_rate" value="1" {{ request('multi_rate') ? 'checked' : '' }}>
                            <label class="form-check-label" for="multi_rate">Show Only Multi Rate Items</label>
                        </div>
                    </div>
                </div>
    ...
    <div class="card mt-3">
        <div class="card-header bg-primary text-white py-2 d-flex justify-content-between">
            <strong>Multi Rate Items Report</strong>
            <div>
...
function exportToExcel() {
    ...
    window.location.href = '{{ route("admin.reports.inventory.item.multi-rate") }}?' + params.toString();
}

function printReport() {
    const params = new URLSearchParams(new FormData(document.getElementById('filterForm')));
    params.set('print', '1');
    window.open('{{ route("admin.reports.inventory.item.multi-rate") }}?' + params.toString(), 'PrintReport', 'width=900,height=700');
}
