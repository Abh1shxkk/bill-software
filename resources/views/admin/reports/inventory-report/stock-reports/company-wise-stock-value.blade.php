...
@section('title', 'Company Wise Stock Value')

@section('content')
<div class="d-flex justify-content-between align-items-start mb-3 flex-wrap gap-2">
    <div class="d-flex align-items-start gap-3">
        <div style="min-width: 100px;">
            <h4 class="mb-0 d-flex align-items-center"><i class="bi bi-building me-2"></i> Company Wise Stock Value</h4>
            <div class="text-muted small">View stock value by company</div>
        </div>
    </div>
...
<div class="card shadow-sm border-0 rounded">
    <div class="card mb-4">
        <div class="card-body">
            <form method="GET" action="{{ route('admin.reports.inventory.stock.company-wise-stock-value') }}" class="row g-3" id="filter-form">
                <div class="col-md-2">
                ...
                <div class="col-md-2">
                    ...
                    <select name="order_by" class="form-select">
                        ...
                        <option value="purchase" {{ request('order_by') == 'purchase' ? 'selected' : '' }}>Purchase</option>
                        <option value="sale" {{ request('order_by') == 'sale' ? 'selected' : '' }}>Sale</option>
                ...
                <div class="col-md-4">
                    ...
                    <div class="input-group">
                        <input type="text" class="form-control" id="search-input" name="search" value="{{ request('search') }}" placeholder="Search company...">
        ...
        <table class="table align-middle mb-0">
            ...
            <tbody id="table-body">
                @if(isset($reportData) && $reportData->count() > 0)
                    ...
                    @endforeach
                @else
                    <tr><td colspan="6" class="text-center text-muted">No data found. Click "View" to load report.</td></tr>
                @endif
            </tbody>
            @if(isset($reportData) && $reportData->count() > 0)
...
function exportToExcel() {
    ...
    window.location.href = '{{ route("admin.reports.inventory.stock.company-wise-stock-value") }}?' + params.toString();
}

function printReport() {
    const params = new URLSearchParams(new FormData(document.getElementById('filter-form')));
    params.set('print', '1');
    window.open('{{ route("admin.reports.inventory.stock.company-wise-stock-value") }}?' + params.toString(), 'PrintReport', 'width=900,height=700');
}
</script>
@endpush
