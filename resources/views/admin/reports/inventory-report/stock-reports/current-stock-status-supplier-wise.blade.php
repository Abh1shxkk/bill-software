...
@section('title', 'Current Stock Status Supplier Wise')

@section('content')
<div class="container-fluid">
    <div class="card mb-2" style="background-color: #ffcc80;">
        <div class="card-body py-2 text-center">
            <h4 class="mb-0 text-primary fst-italic fw-bold" style="font-family: serif; letter-spacing: 1px;">Current Stock Status Supplier Wise</h4>
        </div>
    </div>

    <div class="card shadow-sm" style="background-color: #ffffcc;">
        <div class="card-body p-3">
            <form method="GET" id="filterForm" action="{{ route('admin.reports.inventory.stock.current-stock-status-supplier-wise') }}">
                <div class="row g-2 mb-2 align-items-center">
                    <div class="col-md-2 text-end pe-2">
                        <label class="fw-bold mb-0">Supplier :</label>
                    </div>
                    <div class="col-md-1">
                        <input type="text" name="supplier_code" class="form-control form-control-sm text-center" value="{{ request('supplier_code', '00') }}" style="width: 50px;">
                    </div>
                    <div class="col-md-3">
                    ...
                    <div class="col-md-2">
                        <select name="value_on" class="form-select form-select-sm">
                            <option value="1" {{ request('value_on') == '1' ? 'selected' : '' }}>Cost Rate</option>
                            <option value="2" {{ request('value_on') == '2' ? 'selected' : '' }}>Sale Rate</option>
                            <option value="3" {{ request('value_on') == '3' ? 'selected' : '' }}>Pur. Rate</option>
                            <option value="4" {{ request('value_on') == '4' ? 'selected' : '' }}>MRP</option>
                        </select>
                    </div>
                </div>
                <div class="row g-2 mb-2 align-items-center">
    ...
    <div class="card mt-2">
        ...
        <div class="card-header bg-primary text-white py-2 d-flex justify-content-between">
            <strong>Current Stock Status Supplier Wise</strong>
            <div>
            ...
            <table class="table table-bordered table-sm mb-0" style="background-color: #fff;">
                <thead style="background-color: #ccccff;">
                    <tr>
                        ...
                        <th style="background-color: #ccccff;">Item Name</th>
                        <th style="background-color: #ccccff;">Company</th>
                        <th style="background-color: #ccccff;">Supplier</th>
...
function exportToExcel() {
    ...
    window.location.href = '{{ route("admin.reports.inventory.stock.current-stock-status-supplier-wise") }}?' + params.toString();
}
function printReport() {
    const params = new URLSearchParams(new FormData(document.getElementById('filterForm')));
    params.set('print', '1');
    window.open('{{ route("admin.reports.inventory.stock.current-stock-status-supplier-wise") }}?' + params.toString(), 'PrintReport', 'width=900,height=700');
}
</script>
@endpush
