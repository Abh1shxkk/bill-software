...
<div class="container-fluid">
    ...
    <div class="card mb-2" style="background-color: #ffc4d0;">
        <div class="card-body py-2 text-center">
            <h4 class="mb-0 text-primary fst-italic fw-bold" style="font-family: 'Times New Roman', serif;">SHORTAGE REPORT</h4>
        </div>
    </div>
    ...
    <div class="card shadow-sm mb-2" style="background-color: #f0f0f0;">
        <div class="card-body py-2">
            <form method="GET" id="filterForm" action="{{ route('admin.reports.sales.shortage-report') }}">
                <div class="row g-2 align-items-end">
                    <!-- Row 1: Date Range, Company, Format -->
                    <div class="col-md-2">
                    ...
                    <div class="col-md-2">
                        <div class="input-group input-group-sm">
                            ...
                            <select name="report_format" class="form-select text-uppercase">
                                <option value="D" {{ ($reportFormat ?? 'D') == 'D' ? 'selected' : '' }}>D(etailed)</option>
                                <option value="S" {{ ($reportFormat ?? '') == 'S' ? 'selected' : '' }}>S(ummarized)</option>
                            </select>
                        </div>
                    </div>
                </div>
    ...
    @if(request()->has('view') && isset($shortageItems) && count($shortageItems) > 0)
    <!-- Summary Cards -->
...
function printReport() {
    window.open('{{ route("admin.reports.sales.shortage-report") }}?print=1&' + $('#filterForm').serialize(), '_blank');
}
