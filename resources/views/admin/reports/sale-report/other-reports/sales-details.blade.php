...
<div class="container-fluid">
    ...
    <div class="card mb-2" style="background: linear-gradient(135deg, #f8d7da 0%, #f5c6cb 100%);">
        <div class="card-body py-2 text-center">
            <h4 class="mb-0 fst-italic fw-bold" style="color: #1a0dab; font-family: 'Times New Roman', serif;">-: Sales Details :-</h4>
        </div>
    </div>
    ...
    <div class="card shadow-sm mb-2">
        <div class="card-body py-2" style="background-color: #e9ecef;">
            <form method="GET" id="filterForm" action="{{ route('admin.reports.sales.other.sales-details') }}">
                <div class="row g-2 align-items-end">
                    ...
                    <div class="col-md-2">
                        <select name="customer_id" class="form-select form-select-sm">
                            ...
                            <option value="{{ $c->id }}" {{ ($customerId ?? '') == $c->id ? 'selected' : '' }}>{{ $c->code }} - {{ $c->name }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="col-md-2">
                    ...
                    <div class="col-md-2">
                        <div class="form-check form-check-inline">
                            ...
                            <label class="form-check-label" for="include_cancelled">Cancelled Invoices</label>
                        </div>
                    </div>
                    <div class="col-md-1">
            ...
            <div class="table-responsive" style="max-height: 55vh;">
                <table class="table table-sm table-hover table-striped table-bordered mb-0">
                    ...
                    <tbody>
                        ...
                        <tr class="{{ ($d['status'] ?? '') == 'cancelled' ? 'table-danger' : '' }}">
                            ...
                            <td class="text-end text-success">{{ number_format($d['free_qty'], 0) }}</td>
                            <td class="text-end">{{ number_format($d['rate'], 2) }}</td>
                            <td class="text-end fw-bold">{{ number_format($d['amount'], 2) }}</td>
                        </tr>
                        @empty
                        <tr>
...
function exportToExcel() {
    ...
    window.open('{{ route("admin.reports.sales.other.sales-details") }}?' + params.toString(), '_blank');
}

function viewReport() {
    const params = new URLSearchParams(new FormData(document.getElementById('filterForm')));
    params.set('view_type', 'print');
    window.open('{{ route("admin.reports.sales.other.sales-details") }}?' + params.toString(), 'SalesDetails', 'width=1100,height=800,scrollbars=yes,resizable=yes');
}
</script>
@endpush
