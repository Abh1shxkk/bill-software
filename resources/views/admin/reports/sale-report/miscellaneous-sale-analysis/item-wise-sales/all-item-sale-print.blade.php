...
<head>
    ...
    <title>All Item Sale Report</title>
    <style>
...
<body>
    <div class="header">
        <h2>ALL ITEM SALE REPORT</h2>
        <p>From: {{ $dateFrom ?? date('Y-m-d') }} To: {{ $dateTo ?? date('Y-m-d') }}</p>
    </div>

    <div class="filters">
        <span>Transaction Type: {{ request('transaction_type', '3') == '1' ? 'Sale' : (request('transaction_type') == '2' ? 'Sale Return' : 'Both') }}</span>
    </div>
