<!DOCTYPE html>
<html lang="en"><head><meta charset="UTF-8"><title>Customer Wise Sale - Company Wise</title>
...
<div class="header"><h2>Customer Wise Sale - Company Wise</h2><p>{{ \Carbon\Carbon::parse($dateFrom)->format('d-M-Y') }} To {{ \Carbon\Carbon::parse($dateTo)->format('d-M-Y') }}</p></div>
<table><thead><tr><th>Customer / Company</th><th class="text-right">Qty</th><th class="text-right">Free</th><th class="text-right">Amount</th></tr></thead>
...
<tr><td style="padding-left: 15px;">{{ $row['company_name'] }}</td><td class="text-right">{{ $row['qty'] }}</td><td class="text-right">{{ $row['free_qty'] }}</td><td class="text-right">{{ number_format($row['amount'], 2) }}</td></tr>@endforeach
