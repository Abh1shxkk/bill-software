<!DOCTYPE html>
<html lang="en"><head><meta charset="UTF-8"><title>Customer Wise Sale - Quantity Wise Summary</title>
...
<div class="header"><h2>Customer Wise Sale - Quantity Wise Summary</h2><p>{{ \Carbon\Carbon::parse($dateFrom)->format('d-M-Y') }} To {{ \Carbon\Carbon::parse($dateTo)->format('d-M-Y') }}</p></div>
<table><thead><tr><th>Customer</th><th class="text-right">Total Qty</th><th class="text-right">Free Qty</th><th class="text-right">Total Amount</th></tr></thead>
<tbody>@foreach($data as $row)<tr><td>{{ $row['customer_name'] }}</td><td class="text-right">{{ $row['qty'] }}</td><td class="text-right">{{ $row['free_qty'] }}</td><td class="text-right">{{ number_format($row['amount'], 2) }}</td></tr>@endforeach
