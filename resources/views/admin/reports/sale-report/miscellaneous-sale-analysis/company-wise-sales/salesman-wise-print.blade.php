<!DOCTYPE html>
<html lang="en"><head><meta charset="UTF-8"><title>Company Wise Sales - Salesman Wise</title>
...
<div class="header"><h2>Company Wise Sales - Salesman Wise</h2><p>{{ \Carbon\Carbon::parse($dateFrom)->format('d-M-Y') }} To {{ \Carbon\Carbon::parse($dateTo)->format('d-M-Y') }}</p></div>
<table><thead><tr><th>Company / Salesman</th><th class="text-right">Qty</th><th class="text-right">Free</th><th class="text-right">Amount</th></tr></thead>
...
<tr><td style="padding-left: 15px;">{{ $row['salesman_name'] }}</td><td class="text-right">{{ $row['qty'] }}</td><td class="text-right">{{ $row['free_qty'] }}</td><td class="text-right">{{ number_format($row['amount'], 2) }}</td></tr>@endforeach
