...
<html>
<head>
    <title>Day Purchase Summary - Item Wise</title>
    <style>
...
<body onload="window.print()">
    <div class="header">
        <h3>Day Purchase Summary - Item Wise</h3>
    ...
    <table>
        ...
        <tfoot>
            <tr class="total-row">
                ...
                <td class="text-center">{{ number_format($totals['qty'] ?? 0, 0) }}</td>
                @if(($showValue ?? 'Y') == 'Y')
                <td class="text-right">{{ number_format($totals['value'] ?? 0, 2) }}</td>
                @endif
    ...
    <div class="footer">
        TOTAL PURCHASE VALUE: {{ number_format($totals['value'] ?? 0, 2) }}
    </div>
</body>
</html>
