...
<html>
<head>
    <title>Free Issues Without Qty Report</title>
    <style>
...
<body onload="window.print()">
    ...
    <div class="header">
        <h3>Free Issues Without Qty.</h3>
        <p>Period: {{ \Carbon\Carbon::parse($dateFrom)->format('d/m/Y') }} to {{ \Carbon\Carbon::parse($dateTo)->format('d/m/Y') }}</p>
        <p>Report Type: {{ $reportType == 'D' ? 'Detailed' : 'Summarised' }}</p>
    </div>
    ...
    <table>
        ...
        <tbody>
            ...
            <tr>
                ...
                <td>{{ $item->packing ?? '-' }}</td>
                <td class="text-right text-muted">{{ number_format($item->qty ?? 0, 0) }}</td>
