...
<html>
<head>
    <title>All Item Purchase Report</title>
    <style>
...
<body onload="window.print()">
    ...
    <div class="header">
        <h3>All Item Purchase</h3>
        <p>Period: {{ \Carbon\Carbon::parse($dateFrom)->format('d/m/Y') }} to {{ \Carbon\Carbon::parse($dateTo)->format('d/m/Y') }}</p>
        @if($companyName ?? false)<p>Company: {{ $companyName }}</p>@endif
    </div>
    <table>
        <thead>
            <tr>
                ...
                <th class="text-right">Bonus Qty</th>
        ...
        <tbody>
            @php $sno = 0; $grandQty = 0; $grandFree = 0; $grandAmount = 0; $grandTax = 0; $grandNet = 0; @endphp
            @forelse($items ?? [] as $item)
            @php $grandQty += $item->total_qty; $grandFree += $item->total_free_qty; $grandAmount += $item->total_amount; $grandTax += $item->total_tax; $grandNet += $item->total_net; @endphp
            <tr>
