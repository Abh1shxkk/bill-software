...
<div class="container-fluid">
    ...
    <div class="card mb-2" style="background: linear-gradient(135deg, #d1ecf1 0%, #e8f4f8 100%);">
        <div class="card-body py-2 text-center">
            <h4 class="mb-0 text-info fst-italic fw-bold">TDS OUTPUT</h4>
        </div>
    </div>
        ...
        <div class="card-body py-2">
            <form method="GET" id="filterForm">
                <div class="row g-2">
                    ...
                    <div class="col-md-2">
                        <div class="input-group input-group-sm">
                            <span class="input-group-text">TDS Section</span>
        ...
        <div class="col-md-3">
            <div class="card bg-primary text-white">
                <div class="card-body py-2 px-3">
                    ...
                    <h5 class="mb-0">{{ number_format($totals['transactions'] ?? 0) }}</h5>
                </div>
            </div>
        </div>
        <div class="col-md-3">
            <div class="card bg-info text-white">
                <div class="card-body py-2 px-3">
                    ...
                    <h5 class="mb-0">₹{{ number_format($totals['gross'] ?? 0, 2) }}</h5>
                </div>
            </div>
        </div>
        <div class="col-md-3">
            <div class="card bg-danger text-white">
                <div class="card-body py-2 px-3">
                    <small class="text-white-50">TDS Deducted</small>
                    <h5 class="mb-0">₹{{ number_format($totals['tds'] ?? 0, 2) }}</h5>
                </div>
            </div>
        </div>
        <div class="col-md-3">
            <div class="card bg-success text-white">
                <div class="card-body py-2 px-3">
                    <small class="text-white-50">Net Payable</small>
                ...
                <table class="table table-sm table-hover table-striped table-bordered mb-0">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td><span class="badge bg-info">{{ $record->tds_section ?? '194Q' }}</span></td>
                            <td class="text-end">{{ number_format($record->gross_amount ?? 0, 2) }}</td>
                            <td class="text-center">{{ $record->tds_rate ?? 0 }}%</td>
                            <td class="text-end text-danger fw-bold">{{ number_format($record->tds_amount ?? 0, 2) }}</td>
                            <td class="text-end fw-bold">{{ number_format($record->net_amount ?? 0, 2) }}</td>
                        </tr>
                        @empty
                        <tr>
