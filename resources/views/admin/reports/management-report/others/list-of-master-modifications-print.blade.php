...
<html>
<head>
    <title>List of Master Modifications - Print</title>
    <style>
...
<body onload="window.print()">
    <div class="header">
        <h3>List of Master Modifications</h3>
    ...
    <table>
        ...
        <tbody>
            ...
            <tr>
                ...
                <td>{{ $row['code'] }}</td>
                <td>{{ $row['name'] }}</td>
                <td class="{{ $row['action'] == 'Deleted' ? 'text-danger' : ($row['action'] == 'Created' ? 'text-success' : 'text-warning') }}">{{ $row['action'] }}</td>
