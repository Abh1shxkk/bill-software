...
function loadVoucher(id) {
    closeInvoiceModal();
    fetch(`{{ url('admin/sale-return-voucher') }}/${id}/details`).then(r => r.json()).then(res => { if (res.success) populateForm(res.voucher); else alert('Error: ' + res.message); });
}

function searchVoucher() {
    const inv = document.getElementById('searchInvoice').value.trim();
    if (!inv) { alert('Enter invoice number'); return; }
    fetch(`{{ route('admin.sale-return-voucher.search') }}?invoice_no=${encodeURIComponent(inv)}`).then(r => r.json()).then(res => { if (res.success) populateForm(res.voucher); else alert('Not found'); });
}
