...
@section('title', 'Sale Return Voucher Modification')
                    ...
                    <div class="field-group">
                        ...
                        <div class="searchable-dropdown" id="customerDropdownWrapper" style="width: 250px;">
                            ...
                            <div class="searchable-dropdown-list" id="customerDropdownList" style="display: none;">
                                ...
                                <div class="dropdown-item" data-value="{{ $customer->id }}" data-name="{{ $customer->name }}">{{ $customer->name }}</div>
                                @endforeach
                            </div>
                        </div>
                    </div>
...
function loadVouchersForModal() {
    fetch('{{ route("admin.sale-return-voucher.get-vouchers") }}')
...
function loadVoucher(id) { closeInvoiceModal(); fetch(`{{ url('admin/sale-return-voucher') }}/${id}/details`).then(r => r.json()).then(res => { if (res.success) populateForm(res.voucher); else alert('Error: ' + res.message); }); }
function searchVoucher() { const inv = document.getElementById('searchInvoice').value.trim(); if (!inv) { alert('Enter invoice number'); return; } fetch(`{{ route('admin.sale-return-voucher.search') }}?invoice_no=${encodeURIComponent(inv)}`).then(r => r.json()).then(res => { if (res.success) populateForm(res.voucher); else alert('Not found'); }); }
...
<script>
// ============================================================
// AUTO-SAVE  —  sale_return_voucher_modification_autosave_v1
// ============================================================
(function(){
'use strict';
const KEY = 'sale_return_voucher_modification_autosave_v1';
let _t = null;
...
function save(){
    ...
    document.querySelectorAll('#hsnTableBody tr').forEach(function(tr){
        ...
        if(!gc('hsn-code') && !(parseFloat(gc('amount'))>0)) return;
        rows.push({
            ri:ri,
            hsnCode:gc('hsn-code'), amount:gc('amount'), gstPercent:gc('gst-percent'),
            cgstPercent:gc('cgst-percent'), cgstAmount:gc('cgst-amount'),
            sgstPercent:gc('sgst-percent'), sgstAmount:gc('sgst-amount'), qty:gc('qty'),
        });
    });
    const vId=(typeof currentVoucherId!=='undefined')?currentVoucherId:'';
    const state={
        savedAt:new Date().toISOString(),
        currentVoucherId:vId,
        voucherId:_val('voucherId'),
        returnDate:_val('returnDate'),
        customerId:_val('customerSelect'),
        customerDisplay:_val('customerSearchInput'),
        remarks:_val('remarks'),
        rows:rows,
    };
    if(!vId && !state.voucherId) return;
...
function restore(){
    ...
    if(!state||(!state.currentVoucherId&&!state.voucherId)) return;
    _banner(state.savedAt, function keep(){
        try{ if(state.currentVoucherId) window.currentVoucherId=state.currentVoucherId; }catch(e){}
        _set('voucherId',state.voucherId||'');
        if(state.returnDate) _set('returnDate',state.returnDate);
        _set('customerSelect',state.customerId||'');
        _set('customerSearchInput',state.customerDisplay||'');
        ...
        (state.rows||[]).forEach(function(saved){
            ...
            if(typeof rowCounter!=='undefined'&&ri>=rowCounter) window.rowCounter=ri;
            const tr=document.createElement('tr');
            tr.setAttribute('data-row',ri);
            tr.innerHTML=
                '<td><input type="text" class="hsn-code" data-custom-enter="true" onclick="selectRow('+ri+')" onfocus="selectRow('+ri+')" placeholder="HSN"></td>'+
                '<td><input type="number" class="amount" step="0.01" data-custom-enter="true" onchange="calculateRowTax('+ri+')" oninput="calculateRowTax('+ri+')"></td>'+
                '<td><input type="number" class="gst-percent" step="0.01" data-custom-enter="true" onchange="calculateRowTax('+ri+')"></td>'+
                '<td><input type="number" class="cgst-percent" readonly style="background:#e9ecef;"></td>'+
                '<td><input type="number" class="cgst-amount" readonly style="background:#e9ecef;"></td>'+
                '<td><input type="number" class="sgst-percent" readonly style="background:#e9ecef;"></td>'+
                '<td><input type="number" class="sgst-amount" readonly style="background:#e9ecef;"></td>'+
                '<td><input type="number" class="qty" value="0" min="0" data-custom-enter="true"></td>'+
                '<td class="text-center"><button type="button" class="btn btn-sm btn-outline-danger" onclick="deleteRow('+ri+')"><i class="bi bi-trash"></i></button></td>';
            if(tbody) tbody.appendChild(tr);
            tr.querySelector('.hsn-code').value    = saved.hsnCode||'';
            tr.querySelector('.amount').value       = saved.amount||'';
            tr.querySelector('.gst-percent').value  = saved.gstPercent||'';
            tr.querySelector('.cgst-percent').value = saved.cgstPercent||'';
            tr.querySelector('.cgst-amount').value  = saved.cgstAmount||'';
            tr.querySelector('.sgst-percent').value = saved.sgstPercent||'';
            tr.querySelector('.sgst-amount').value  = saved.sgstAmount||'';
            tr.querySelector('.qty').value          = saved.qty||'0';
        });
