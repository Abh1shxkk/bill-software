<?php $__env->startSection('title', 'Pending Orders - ' . $supplier->name); ?>
<?php $__env->startSection('content'); ?>
<div class="d-flex justify-content-between align-items-center mb-3">
    <div>
        <h4 class="mb-0 d-flex align-items-center">
            <i class="bi bi-cart-check me-2"></i> Pending Orders - <?php echo e($supplier->name); ?>

        </h4>
        <div class="text-muted small">Manage pending orders for this supplier</div>
    </div>
    <div>
        <a href="<?php echo e(route('admin.suppliers.show', $supplier)); ?>" class="btn btn-outline-secondary btn-sm">
            <i class="bi bi-arrow-left"></i> Back
        </a>
        <?php
            $uniqueOrderNos = $orders->pluck('order_no')->unique()->filter();
        ?>
        <?php if($uniqueOrderNos->isNotEmpty()): ?>
        ...
        <form id="addOrderForm">
            ...
            <div class="row g-2 mb-2">
                ...
                <div class="col-md-4">
                    ...
                    <select name="item_id" id="itemSelect" class="form-select form-select-sm" required onchange="handleItemSelectChange()">
                        <option value="">-- Select Item --</option>
                        <?php $__currentLoopData = $items; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $item): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <option value="<?php echo e($item->id); ?>" 
                                data-code="<?php echo e($item->bar_code ?: $item->id); ?>" 
                                data-name="<?php echo e($item->name); ?>" 
                                data-pack="<?php echo e($item->packing); ?>"
                                data-company="<?php echo e($item->company_short_name); ?>"
                                data-stock="<?php echo e($item->total_stock); ?>">
                                <?php echo e($item->bar_code ?: $item->id); ?> - <?php echo e($item->name); ?>
                ...
                <div class="col-md-2">
                    ...
                    <input type="number" name="balance_qty" id="balanceQty" class="form-control form-control-sm" value="0" step="1" required readonly style="background-color: #f8f9fa;">
                </div>
                <div class="col-md-2">
                    <label class="form-label small fw-bold mb-1">Order Date</label>
                    <input type="date" name="order_date" class="form-control form-control-sm" value="<?php echo e(now()->toDateString()); ?>" required>
                </div>
            </div>
...
<div class="card shadow-sm">
        ...
        <table class="table table-sm align-middle mb-0" style="font-size: 0.875rem;">
            ...
            <tbody id="orders-table-body">
                ...
                <tr data-order-id="<?php echo e($order->id); ?>" 
                    data-item-cost="<?php echo e($order->item->cost ?? 0); ?>"
                    data-order-qty="<?php echo e($order->order_qty ?? 0); ?>"
                    data-free-qty="<?php echo e($order->free_qty ?? 0); ?>"
                    data-item-name="<?php echo e($order->item->name ?? '---'); ?>"
                    class="order-row clickable-row"
                    style="cursor: pointer;">
                    <td class="text-center"><?php echo e($index + 1); ?></td>
                    <td class="text-center"><span class="badge bg-primary"><?php echo e($order->order_no ?? '---'); ?></span></td>
                    <td class="text-center"><?php echo e($order->item->company_short_name ?? '---'); ?></td>
                    <td class="text-center"><?php echo e($order->balance_qty ?? 0); ?></td>
                    <td class="text-center"><?php echo e($order->item->bar_code ?? '---'); ?></td>
                    <td><?php echo e($order->item->name ?? '---'); ?></td>
                    <td class="text-center"><?php echo e($order->item->packing ?? '---'); ?></td>
                    <td class="text-center"><?php echo e($order->order_qty ?? 0); ?></td>
                    <td class="text-center"><?php echo e($order->free_qty ?? 0); ?></td>
                    <td class="text-center"><?php echo e($order->other_order ?? 0); ?></td>
                    <td class="text-center"><?php echo e($order->order_date ? \Carbon\Carbon::parse($order->order_date)->format('d-M-y') : '---'); ?></td>
                    <td class="text-end">
                        <a href="<?php echo e(route('admin.items.pending-orders', $order->item_id)); ?>" 
                        ...
                        <button type="button" class="btn btn-sm btn-outline-danger delete-order" 
                            ...
                            data-item-name="<?php echo e($order->item->name ?? 'Unknown Item'); ?>" 
                            title="Delete">
    ...
    <div class="card-footer">
        <div class="row">
            <div class="col-md-4">
                <div class="text-muted small">Total Orders: <span id="total-orders"><?php echo e($orders->count()); ?></span></div>
            </div>
            ...
            <div class="col-md-4 text-end">
                ...
                <span class="badge bg-success fs-6" id="total-value">₹<?php echo e(number_format($orders->sum(function($order) { return ($order->item->cost ?? 0) * (($order->order_qty ?? 0) + ($order->free_qty ?? 0)); }), 2)); ?></span>
            </div>
        </div>
    </div>
</div>
            ...
            <form id="printOrderForm">
                <div class="mb-3">
                    ...
                    <select class="form-select" id="printOrderNo" required>
                        ...
                        <?php $__currentLoopData = $uniqueOrderNos; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $orderNo): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <option value="<?php echo e($orderNo); ?>"><?php echo e($orderNo); ?></option>
                        <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                    </select>
                </div>
                <div class="mb-3">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Auto-populate item fields - Global function for inline onchange
    window.handleItemSelectChange = function() {
        const itemSelect = document.getElementById('itemSelect');
        ...
        if (itemSelect.value) {
            ...
            document.getElementById('balanceQty').value = selectedOption.getAttribute('data-stock') || 0;
            
            // Focus on Order Qty field for user input
            document.querySelector('input[name="order_qty"]').focus();
        } else {
            // Clear all fields when no item selected
            document.getElementById('itemCode').value = '';
            document.getElementById('itemName').value = '';
            document.getElementById('itemPack').value = '';
            document.getElementById('itemCompany').value = '';
            document.getElementById('balanceQty').value = 0;
        }
    };
    ...
    form.addEventListener('submit', function(e) {
        ...
        const url = '<?php echo e(route("admin.suppliers.pending-orders.store", $supplier)); ?>';
        ...
        if (e.target.closest('.delete-order')) {
            ...
            window.deleteModal.show({
                ...
                onConfirm: function() {
                    ...
                    fetch(`<?php echo e(route("admin.suppliers.pending-orders.delete", [$supplier, "ORDER_ID"])); ?>`.replace('ORDER_ID', orderId), {
                        method: 'DELETE',
                        headers: {
                            'X-Requested-With': 'XMLHttpRequest',
                            'X-CSRF-TOKEN': '<?php echo e(csrf_token()); ?>',
    ...
    document.getElementById('printOrderForm').addEventListener('submit', function(e) {
        ...
        const baseUrl = '<?php echo e(route("admin.suppliers.pending-orders.print", [$supplier, "ORDER_NO_PLACEHOLDER"])); ?>';
...
<?php echo $__env->make('layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\bill-software\resources\views/admin/suppliers/pending-orders.blade.php ENDPATH**/ ?>