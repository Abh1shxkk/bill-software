<?php $__env->startSection('title', 'Purchase Return Br.Expiry Adjustment'); ?>
...
<div class="bsi-form">
    <div class="d-flex justify-content-between align-items-center mb-2">
        ...
        <div class="d-flex gap-2">
            ...
            <a href="<?php echo e(route('admin.breakage-supplier.received-modification')); ?>" class="btn btn-outline-secondary btn-sm py-0">
            ...
            <div class="row g-2">
                <div class="col-md-2">
                    <div class="field-group">
                        ...
                        <input type="date" id="transaction_date" name="transaction_date" class="form-control" value="<?php echo e(date('Y-m-d')); ?>">
                    </div>
                    <div class="field-group">
                        <label style="width:35px;">Trn.No.</label>
                        <input type="text" id="trn_no" name="trn_no" class="form-control readonly-field" value="<?php echo e($trnNo ?? ''); ?>" readonly style="width:80px;">
                    </div>
                </div>
                <div class="col-md-10">
                                ...
                                <div class="field-group">
                                    ...
                                    <div class="custom-dropdown-wrapper" style="width: 100%; position: relative;">
                                        ...
                                        <select id="supplier_id" name="supplier_id" class="form-control" style="display: none;">
                                            ...
                                            <option value="<?php echo e($s->supplier_id); ?>" data-name="<?php echo e($s->name); ?>"><?php echo e($s->name); ?></option>
                                            ...
                                            <div id="supplierList" class="dropdown-list">
                                                <?php $__currentLoopData = $suppliers ?? []; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $s): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                                    <div class="dropdown-item"
                                                         data-id="<?php echo e($s->supplier_id); ?>"
...
function loadClaims() {
    fetch('<?php echo e(route("admin.claim-to-supplier.past-claims")); ?>')
        .then(r => r.json())
        .then(data => {
...
function loadHsnCodes() {
    fetch('<?php echo e(route("admin.sale-voucher.hsn-codes")); ?>')
        .then(r => r.json())
        .then(data => {
...
function showAdjustModal() {
    ...
    const url = `<?php echo e(url('admin/breakage-supplier/supplier-purchases')); ?>/${supplierId}`;
...
function submitTransaction(adjustments) {
    ...
    fetch('<?php echo e(route("admin.breakage-supplier.store-received")); ?>', {
        method: 'POST',
        headers: {
...
function cancelTransaction() {
    if (confirm('Discard changes?')) {
        window.location.href = '<?php echo e(route("admin.breakage-supplier.received-modification")); ?>';
    }
}
...
<script>
// ============================================================
// AUTO-SAVE  —  bsi_received_transaction_autosave_v1
// ============================================================
(function(){
'use strict';
const KEY = 'bsi_received_transaction_autosave_v1';
let _t = null;
...
function save(){
    ...
    const hasData = state.supplier_id || state.party_trn_no || parseFloat(state.claim_amount||0)>0;
    if(!hasData) return;
...
function restore(){
    ...
    if(!state||(!state.supplier_id&&!state.party_trn_no)) return;
    _banner(state.savedAt, function keep(){
        ...
        if(typeof calculateFromClaimAmount==='function') calculateFromClaimAmount();
...
document.addEventListener('DOMContentLoaded',function(){
    // Patch window.location.reload to clear draft on success
    setTimeout(function(){
...
<?php $__env->stopPush(); ?>
<?php echo $__env->make('layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\bill-software\resources\views/admin/breakage-supplier/received-transaction.blade.php ENDPATH**/ ?>