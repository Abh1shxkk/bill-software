...
<head>
    ...
    <meta name="base-url" content="<?php echo e(rtrim(config('app.url'), '/')); ?>">
</head>
...
<body>
        ...
        <aside class="sidebar p-3 position-relative">
            <div class="sidebar-header">
                <div class="brand d-flex align-items-center">
                    <img src="<?php echo e(asset('https://res.cloudinary.com/dz8p5iadt/image/upload/v1766408873/m-logo-01_hdqtci.svg')); ?>" alt="Medi BillSuite" style="height: 32px;" class="sidebar-logo">
            ...
            <div class="profile">
                <div class="dropup w-100">
                    <button class="btn w-100 d-flex align-items-center text-white" data-bs-toggle="dropdown"
                        ...
                        <img src="<?php echo e(auth()->user()->profile_picture ? (str_starts_with(auth()->user()->profile_picture, 'storage/') ? asset(auth()->user()->profile_picture) : asset('storage/' . auth()->user()->profile_picture)) : 'https://i.pravatar.cc/32'); ?>"
                            class="rounded-circle me-2" width="28" height="28" alt="profile" onerror="this.src='https://i.pravatar.cc/32'">
                        <span class="flex-grow-1 text-truncate label text-start small"><?php echo e(auth()->user()->full_name); ?></span>
                        <i class="bi bi-chevron-up ms-auto small"></i>
                    </button>
                    <ul class="dropdown-menu dropdown-menu-dark">
                        <li class="px-3 py-1">
                            <a class="btn btn-outline-light w-100 d-flex align-items-center justify-content-center" href="<?php echo e(route('profile.settings')); ?>">
    ...
    <script>
        ...
        $(document).ready(function() {
            // Initialize Select2 on all select elements EXCEPT cash and transfer
            ...
            $(document).on('DOMNodeInserted', function(e) {
                if ($(e.target).is('select') || $(e.target).find('select').length) {
                    $(e.target).find('select').not('.no-select2, .select2-hidden-accessible, #cash, #transfer').select2({
                        theme: 'bootstrap-5',
                        width: '100%',
                        placeholder: 'Select an option',
                        allowClear: true
        ...
        $(document).ready(function () {
            ...
            function initializeSelect2(container) {
                const selectElements = container 
                    ? $(container).find('select:not(.select2-hidden-accessible)') 
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            <?php if(session('crud_success')): ?>
                crudNotification.showToast('success', 'Success', '<?php echo e(session('crud_success')); ?>');
            <?php endif; ?>

            <?php if(session('crud_error')): ?>
                crudNotification.showToast('error', 'Error', '<?php echo e(session('crud_error')); ?>');
            <?php endif; ?>

            <?php if(session('crud_warning')): ?>
                crudNotification.showToast('warning', 'Warning', '<?php echo e(session('crud_warning')); ?>');
            <?php endif; ?>

            <?php if(session('crud_info')): ?>
                crudNotification.showToast('info', 'Info', '<?php echo e(session('crud_info')); ?>');
            <?php endif; ?>

            // Access Denied toast for permission errors
            <?php if(session('access_denied')): ?>
                crudNotification.showToast('error', 'Access Denied', 'You do not have permission to access "<?php echo e(session('access_denied')); ?>" module. Please contact your administrator.');
            <?php endif; ?>

            // Specific CRUD operation messages
            <?php if(session('item_created')): ?>
                crudNotification.created('<?php echo e(session('item_created')); ?>');
            <?php endif; ?>

            <?php if(session('item_updated')): ?>
                crudNotification.updated('<?php echo e(session('item_updated')); ?>');
            <?php endif; ?>

            <?php if(session('item_deleted')): ?>
                crudNotification.deleted('<?php echo e(session('item_deleted')); ?>');
            <?php endif; ?>
        });
    ...
    <?php echo $__env->make('layouts.partials.keyboard-shortcuts-config', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?>
    <?php echo $__env->make('layouts.partials.index-shortcuts-config', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?>
    <?php echo $__env->make('layouts.partials.keyboard-shortcuts-inline', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?>
    <?php echo $__env->make('layouts.partials.index-shortcuts-inline', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?>
    
    <!-- Transaction Shortcuts - End key to save (excluded from sale transaction which has its own) -->
    <?php if(!request()->routeIs('admin.sale.transaction')): ?>
    <?php echo $__env->make('layouts.partials.transaction-shortcuts', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?>
    <?php endif; ?>
</body>

</html><?php /**PATH C:\xampp\htdocs\bill-software\resources\views/layouts/admin.blade.php ENDPATH**/ ?>