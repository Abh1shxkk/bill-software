...
window.addEventListener('keydown', function(e) {
    ...
    if (e.key === 'Enter' || e.key === 'Tab') {
        ...
        var hasModalOpenForQty =
            (itemModal && itemModal.classList.contains('show')) ||
            (batchModal && batchModal.classList.contains('show')) ||
            (pastModal && pastModal.classList.contains('show'));

        if (!hasModalOpenForQty && activeQty && activeQty.classList && activeQty.classList.contains('qty-input')) {
...
<script>
// ============================================================
// AUTO-SAVE  —  stock_adjustment_modification_autosave_v1
// ============================================================
(function(){
'use strict';
const KEY = 'stock_adjustment_modification_autosave_v1';
let _t = null;
...
function save(){
    ...
    const state={
        ...
        adjustmentId:_val('adjustmentId'),
        adjustmentDate:_val('adjustmentDate'),
        remarks:_val('remarks'),
        rows:rows,
    };
    if(!state.adjustmentId&&!rows.length) return;
...
function restore(){
    ...
    if(!state||!state.adjustmentId) return;
    _banner(state.savedAt, function keep(){
        _set('adjustmentId',state.adjustmentId||'');
        ...
        // Re-enable fields (loaded state)
        const adj=document.getElementById('adjustmentDate'); if(adj) adj.disabled=false;
        const rem=document.getElementById('remarks'); if(rem) rem.disabled=false;
...
document.addEventListener('DOMContentLoaded',function(){
    setTimeout(function(){
        const _origUpdate=(typeof updateTransaction==='function')?updateTransaction:null;
        if(_origUpdate){
            window.updateTransaction=function(){ clearAutoSave(); _origUpdate.apply(this,arguments); };
        }
    },800);
