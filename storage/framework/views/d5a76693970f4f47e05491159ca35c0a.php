...
<div class="card shadow-sm border-0 rounded">
  <div class="card mb-4">
    <div class="card-body">
<form method="GET" action="<?php echo e(route('admin.items.index')); ?>" class="row g-3" id="item-filter-form">
        <div class="col-md-3">
          ...
          <select class="form-select" id="search_field" name="search_field">
            ...
            <option value="name" <?php echo e(request('search_field') == 'name' ? 'selected' : ''); ?>>1. Split Name</option>
            <option value="bar_code" <?php echo e(request('search_field') == 'bar_code' ? 'selected' : ''); ?>>2. BarCode</option>
            <option value="location" <?php echo e(request('search_field') == 'location' ? 'selected' : ''); ?>>3. Location</option>
            <option value="packing" <?php echo e(request('search_field') == 'packing' ? 'selected' : ''); ?>>4. Pack</option>
            <option value="mrp" <?php echo e(request('search_field') == 'mrp' ? 'selected' : ''); ?>>5. Mrp</option>
            <option value="code" <?php echo e(request('search_field') == 'code' ? 'selected' : ''); ?>>6. BtCode</option>
            <option value="hsn_code" <?php echo e(request('search_field') == 'hsn_code' ? 'selected' : ''); ?>>7. HSN</option>
          </select>
        </div>
        <div class="col-md-7">
          <label for="search" class="form-label">Search</label>
          <div class="input-group">
            <input type="text" class="form-control" id="item-search" name="search" value="<?php echo e(request('search')); ?>" placeholder="Type to search..." autocomplete="off">
      ...
      <tbody id="item-table-body">
        <?php $__empty_1 = true; $__currentLoopData = $items; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $item): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
          <tr>
            <td>
              <div class="form-check">
                <input class="form-check-input item-checkbox" type="checkbox" value="<?php echo e($item->id); ?>" id="item-<?php echo e($item->id); ?>">
            ...
            <td><?php echo e($item->packing ?? '-'); ?></td>
            <td><?php echo e($item->company->short_name ?? '-'); ?></td>
            <td>
              <span class="badge bg-info text-white"><?php echo e(number_format($item->total_units ?? 0, 0)); ?></span>
            </td>
            <td class="text-end">
              <a class="btn btn-sm btn-outline-info" href="<?php echo e(route('admin.batches.index', ['item_id' => $item->id])); ?>" title="Available Batches (Non-Zero Qty)"><i class="bi bi-boxes"></i></a>
              <a class="btn btn-sm btn-outline-secondary" href="<?php echo e(route('admin.batches.all', ['item_id' => $item->id])); ?>" title="All Batches (Including Zero Qty)"><i class="bi bi-archive"></i></a>
              <a class="btn btn-sm btn-outline-warning" href="<?php echo e(route('admin.items.stock-ledger-complete', $item->id)); ?>" title="Stock Ledger (F10)"><i class="bi bi-graph-up"></i></a>
              <a class="btn btn-sm btn-outline-danger" href="<?php echo e(route('admin.items.expiry-ledger', $item->id)); ?>" title="Expiry Ledger"><i class="bi bi-clock-history"></i></a>
              <a class="btn btn-sm btn-outline-primary" href="<?php echo e(route('admin.items.show',$item)); ?>" title="View"><i class="bi bi-eye"></i></a>
              <a class="btn btn-sm btn-outline-secondary" href="<?php echo e(route('admin.items.edit',$item)); ?>" title="Edit"><i class="bi bi-pencil"></i></a>
              <form action="<?php echo e(route('admin.items.destroy',$item)); ?>" method="POST" class="d-inline ajax-delete-form">
                <?php echo csrf_field(); ?> <?php echo method_field('DELETE'); ?>
                <button type="button" class="btn btn-sm btn-outline-danger ajax-delete" data-delete-url="<?php echo e(route('admin.items.destroy',$item)); ?>" data-delete-message="Delete this item?" title="Delete"><i class="bi bi-trash"></i></button>
              </form>
            </td>
          </tr>
  ...
  <div class="card-footer bg-light d-flex flex-column gap-2">
    <div class="d-flex justify-content-between align-items-center w-100">
      <div>Showing <?php echo e($items->firstItem() ?? 0); ?>-<?php echo e($items->lastItem() ?? 0); ?> of <?php echo e($items->total()); ?></div>
      <div class="text-muted">Page <?php echo e($items->currentPage()); ?> of <?php echo e($items->lastPage()); ?></div>
    </div>
    <?php if($items->hasMorePages()): ?>
      ...
      <div id="item-sentinel" data-next-url="<?php echo e($items->appends(request()->query())->nextPageUrl()); ?>" style="height: 1px;"></div>
    <?php endif; ?>
  </div>
</div>
...
window.performItemsSearch = function() {
  ...
  fetch(`<?php echo e(route('admin.items.index')); ?>?${params.toString()}`, {
    headers: {
...
function confirmMultipleDelete() {
  ...
  if (typeof window.GlobalMultipleDelete === 'undefined' || !document.getElementById('globalMultipleDeleteModal')) {
    ...
    if (confirm(`Are you sure you want to delete ${selectedItems.length} selected items?\n\n${displayText}\n\nThis action cannot be undone.`)) {
      ...
      fetch('<?php echo e(route("admin.items.multiple-delete")); ?>', {
        method: 'POST',
        headers: {
  ...
  window.GlobalMultipleDelete.show({
    ...
    deleteUrl: '<?php echo e(route("admin.items.multiple-delete")); ?>',
    itemType: 'items',
...
<?php echo $__env->make('layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\bill-software\resources\views/admin/items/index.blade.php ENDPATH**/ ?>