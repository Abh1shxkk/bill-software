<?php $__env->startSection('title', 'Stock Transfer Outgoing Return - Transaction'); ?>
...
@media (max-width: 767px) {
    ...
    .search-section .row.g-2 { flex-wrap: wrap !important; }
    .search-section .col-auto {
        flex: 1 1 100% !important;
        max-width: 100% !important;
    }
    .search-section .col-auto input { width: 100% !important; }
    .search-section .col-auto .btn { width: 100% !important; }
    ...
    .header-section .field-group { flex-wrap: nowrap !important; }
    .header-section .field-group input {
        flex: 1 !important;
        width: auto !important;
        min-width: 0 !important;
    }
    #stor_customerDropdownWrapper,
    #storm_customerDropdownWrapper,
...
<div class="d-flex justify-content-between align-items-center mb-3">
    ...
    <div>
        <a href="<?php echo e(route('admin.stock-transfer-outgoing-return.index')); ?>" class="btn btn-primary btn-sm">
...
function saveTransaction() {
    ...
    fetch('<?php echo e(route("admin.stock-transfer-outgoing-return.transaction.store")); ?>', {
        method: 'POST',
        headers: {
...
<script>
// ============================================================
// AUTO-SAVE  —  stock_transfer_outgoing_return_transaction_autosave_v1
// ============================================================
(function(){
'use strict';
const KEY = 'stock_transfer_outgoing_return_transaction_autosave_v1';
let _t = null;
...
function save(){
    ...
    const state={
        ...
        stor_transaction_date:_val('stor_transaction_date'),
        customerSelect:_val('customerSelect'),
        stor_customerDisplay:_val('stor_customerDisplay'),
        transfer_to_name:_val('transfer_to_name'),
        stor_trf_return_no:_val('stor_trf_return_no'),
        stor_remarks:_val('stor_remarks'),
        stor_gr_no:_val('stor_gr_no'),
        stor_gr_date:_val('stor_gr_date'),
        stor_cases:_val('stor_cases'),
        stor_transport:_val('stor_transport'),
        rows:rows,
    };
...
function restore(){
    ...
    _banner(state.savedAt, function keep(){
        if(state.stor_transaction_date) _set('stor_transaction_date',state.stor_transaction_date);
        if(typeof updateDayName==='function') updateDayName();
        _set('customerSelect',state.customerSelect||'');
        const cd=document.getElementById('stor_customerDisplay'); if(cd) cd.value=state.stor_customerDisplay||'';
        _set('transfer_to_name',state.transfer_to_name||'');
        _set('stor_trf_return_no',state.stor_trf_return_no||'');
        _set('stor_remarks',state.stor_remarks||'');
        _set('stor_gr_no',state.stor_gr_no||'');
        if(state.stor_gr_date) _set('stor_gr_date',state.stor_gr_date);
        _set('stor_cases',state.stor_cases||'0');
        _set('stor_transport',state.stor_transport||'');
        ...
        (state.rows||[]).forEach(function(saved){
            ...
            tr.innerHTML=
                '<td><input type="text" class="form-control" name="items['+ri+'][code]" value="'+_esc(saved.code)+'" onchange="searchItemByCode('+ri+',this.value)" onkeydown="handleCodeKeydown(event,'+ri+')" data-custom-enter></td>'+
                '<td><input type="text" class="form-control" name="items['+ri+'][name]" value="'+_esc(saved.name)+'" readonly></td>'+
                '<td><input type="text" class="form-control" name="items['+ri+'][batch]" value="'+_esc(saved.batch)+'" onkeydown="handleBatchKeydown(event,'+ri+')" data-custom-enter></td>'+
                '<td><input type="text" class="form-control" name="items['+ri+'][expiry]" value="'+_esc(saved.expiry)+'" readonly></td>'+
                '<td><input type="number" class="form-control text-end" name="items['+ri+'][qty]" value="'+_esc(saved.qty||0)+'" min="0" onchange="calculateRowAmount('+ri+')" onkeydown="handleQtyKeydown(event,'+ri+')" data-custom-enter></td>'+
                '<td><input type="number" class="form-control text-end" name="items['+ri+'][rate]" value="'+parseFloat(saved.rate||0).toFixed(2)+'" step="0.01" onchange="calculateRowAmount('+ri+')" onkeydown="handleRateKeydown(event,'+ri+')" data-custom-enter></td>'+
...
<?php $__env->stopPush(); ?>
<?php echo $__env->make('layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\bill-software\resources\views/admin/stock-transfer-outgoing-return/transaction.blade.php ENDPATH**/ ?>