...
@media (max-width: 767px) {
    ...
    /* ── Customer & Salesman custom dropdowns ── */
    #sv_customerDropdownWrapper,
    #sv_salesmanDropdownWrapper {
        width: 100% !important;
        flex: 1 !important;
    }
    .sv-custom-dropdown {
...
<script>
// ============================================================
// AUTO-SAVE  —  sale_voucher_transaction_autosave_v1
// ============================================================
(function(){
'use strict';
const KEY = 'sale_voucher_transaction_autosave_v1';
let _t = null;
...
function save(){
    ...
    const state={
        ...
        customerId:_val('customerId'), customerDisplay:_val('sv_customerDisplay'),
        salesmanId:_val('salesmanId'), salesmanDisplay:_val('sv_salesmanDisplay'),
        cashFlag:_val('cashFlag'), remarks:_val('remarks'),
        rows:rows,
    };
    if(!state.customerId && !rows.length) return;
...
function restore(){
    ...
    if(!state||(!state.customerId&&!state.rows.length)) return;
    _banner(state.savedAt, function keep(){
        ...
        _set('sv_customerDisplay',state.customerDisplay||'');
        _set('salesmanId',state.salesmanId||'');
        _set('sv_salesmanDisplay',state.salesmanDisplay||'');
        ...
        (state.rows||[]).forEach(function(saved){
            ...
            tr.innerHTML=
                '<td><input type="text" class="hsn-code" data-row="'+ri+'" onclick="selectRow('+ri+')" onfocus="selectRow('+ri+')" onchange="onHsnChange(this)" placeholder="Enter HSN" data-custom-enter onkeydown="handleHsnKeydown(event,'+ri+')"></td>'+
                '<td><input type="number" class="amount" step="0.01" onchange="calculateRowTax('+ri+')" oninput="calculateRowTax('+ri+')" placeholder="0.00" data-custom-enter onfocus="selectRow('+ri+')" onkeydown="handleAmountKeydown(event,'+ri+')"></td>'+
                '<td><input type="number" class="gst-percent" step="0.01" onchange="calculateRowTax('+ri+')" placeholder="0" data-custom-enter onfocus="selectRow('+ri+')" onkeydown="handleGstKeydown(event,'+ri+')"></td>'+
                '<td><input type="number" class="cgst-percent" step="0.01" readonly style="background:#e9ecef;"></td>'+
                '<td><input type="number" class="cgst-amount" step="0.01" readonly style="background:#e9ecef;"></td>'+
                '<td><input type="number" class="sgst-percent" step="0.01" readonly style="background:#e9ecef;"></td>'+
                '<td><input type="number" class="sgst-amount" step="0.01" readonly style="background:#e9ecef;"></td>'+
                '<td><input type="number" class="qty" value="0" step="1" min="0" placeholder="0" data-custom-enter onfocus="selectRow('+ri+')" onkeydown="handleQtyKeydown(event,'+ri+')"></td>'+
            ...
            // Restore values after appending
