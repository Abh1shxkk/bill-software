...
function createEmptyRowAfterDiscount(rowIndex) {
    ...
    var currentRow = document.querySelector('#itemsTableBody tr[data-row-index="' + rowIndex + '"]');
    if (currentRow) {
        var nextRow = currentRow.nextElementSibling;
        if (nextRow) {
            var nextName = nextRow.querySelector('input[name*="[item_name]"]');
            if (nextName && nextName.value.trim()) {
                /* Next row has data → focus its Qty field (same loop) */
                var nextQty = nextRow.querySelector('input[name*="[qty]"]');
                if (nextQty) {
                    setTimeout(function() { nextQty.focus(); nextQty.select(); }, 50);
                    return;
                }
            }
        }
    ...
    /* Next row is empty or doesn't exist → add row and focus code */
