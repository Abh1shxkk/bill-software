<?php $__env->startSection('title', 'Breakage/Expiry to Supplier - Issued Modification'); ?>
            ...
            <div class="row g-2">
                ...
                <div class="col-md-10">
                    <div class="inner-card">
                        <div class="row g-2">
                            <div class="col-md-5"><div class="field-group"><label style="width:55px;">Supplier:</label><select id="supplier_id" name="supplier_id" class="form-control no-select2" onchange="updateSupplierName()"><option value="">Select Supplier</option><?php $__currentLoopData = $suppliers ?? []; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $s): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?><option value="<?php echo e($s->supplier_id); ?>" data-name="<?php echo e($s->name); ?>"><?php echo e($s->name); ?></option><?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?></select></div></div>
...
function loadInvoices(search = '') {
    fetch(`<?php echo e(route('admin.breakage-supplier.get-issued-past-invoices')); ?>?search=${encodeURIComponent(search)}`)
        .then(r => r.json())
        .then(invoices => {
            const tbody = document.getElementById('invoicesListBody');
            if (!invoices.length) {
...
function loadInvoice(id) {
    ...
    fetch(`<?php echo e(url('admin/breakage-supplier/issued')); ?>/${id}`)
        .then(r => {
...
function updateTransaction() {
    ...
    fetch(`<?php echo e(url('admin/breakage-supplier/issued')); ?>/${transactionId}`, {
        method: 'POST',
        body: formData,
        headers: { 
...
<?php echo $__env->make('layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\bill-software\resources\views/admin/breakage-supplier/issued-modification.blade.php ENDPATH**/ ?>