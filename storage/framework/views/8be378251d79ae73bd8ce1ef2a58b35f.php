...
<style>
    ...
    /* Blue focus border on entry fields */
    .debit-name:focus, .credit-name:focus, .entry-amount:focus,
    .entry-date:focus, .dr-slcd:focus {
        outline: none !important;
        border: 2px solid #0000ff !important;
        box-shadow: 0 0 0 3px rgba(0,0,255,0.15) !important;
    }
    /* Modal keyboard highlight */
    #accountListBody tr.kb-hi { background: #000080 !important; color: #fff !important; }
    #accountListBody tr.kb-hi td { color: #fff !important; }

    /* Checkbox focus glow */
    #fixedDebit:focus, #fixedCredit:focus, #fixedAmount:focus {
        outline: none !important;
        box-shadow: 0 0 0 4px rgba(0,0,255,0.4) !important;
        border-radius: 3px;
    }
    #fixedDebit:focus + label, #fixedCredit:focus + label, #fixedAmount:focus + label {
        color: #0000ff;
        font-weight: bold;
    }
</style>
...
<div class="custom-modal-overlay" id="accountModalOverlay">
    <div class="custom-modal">
        ...
        <div class="custom-modal-body">
            ...
            <input type="text" class="form-control form-control-sm mb-2" id="accountSearchInput" placeholder="Search..." oninput="filterAccounts()">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    setTimeout(function() { document.getElementById('fixedDebit').focus(); }, 150);
    ...
    function _modalVisibleRows() {
        return Array.from(document.querySelectorAll('#accountListBody tr'))
                    .filter(function(r) { return r.style.display !== 'none'; });
    }
    function _modalHighlight(idx) {
        var rows = _modalVisibleRows();
        document.querySelectorAll('#accountListBody tr').forEach(function(r) { r.classList.remove('table-primary', 'kb-hi'); });
        if (idx >= 0 && idx < rows.length) { rows[idx].classList.add('table-primary', 'kb-hi'); rows[idx].scrollIntoView({ block: 'nearest' }); }
        modalKbIdx = idx;
    }

    /* Window capture: modal keyboard when modal is open */
    var _tabOrder = ['Customer', 'Supplier', 'General'];
    window.addEventListener('keydown', function(e) {
        if (!_isModalOpen()) return;
        var managedKeys = ['ArrowDown', 'ArrowUp', 'ArrowLeft', 'ArrowRight', 'Enter', 'Escape', 'Tab'];
        if (managedKeys.indexOf(e.key) === -1) return;
        // Don't interfere with tab type buttons
        if (e.target.closest('.account-type-tab')) return;
        ...
        if (e.key === 'ArrowDown') {
            ...
            _modalHighlight(modalKbIdx);
            setTimeout(function() { document.getElementById('accountSearchInput').focus(); }, 0);
            return;
        }
        if (e.key === 'ArrowUp') {
            if (rows.length === 0) return;
        ...
        if (e.key === 'Enter') {
            if (modalKbIdx < 0 && rows.length > 0) { modalKbIdx = 0; _modalHighlight(0); return; }
            selectAccount();
            return;
        }
        if (e.key === 'Escape') { closeAccountModal(); return; }
        if (e.key === 'Tab') { document.getElementById('accountSearchInput').focus(); return; }
        if (e.key === 'ArrowLeft' || e.key === 'ArrowRight') {
            var curIdx = _tabOrder.indexOf(currentAccountType);
            if (e.key === 'ArrowLeft') curIdx = (curIdx - 1 + _tabOrder.length) % _tabOrder.length;
            else curIdx = (curIdx + 1) % _tabOrder.length;
            switchAccountType(_tabOrder[curIdx]);
            modalKbIdx = -1;
            document.querySelectorAll('#accountListBody tr').forEach(function(r) { r.classList.remove('table-primary', 'kb-hi'); });
            setTimeout(function() { document.getElementById('accountSearchInput').value = ''; document.getElementById('accountSearchInput').focus(); }, 0);
            return;
        }
    }, true);

    /* Reset modal highlight on search input */
    ...
    /* ══════════════════════════════════════════════════════
       ENTRY TABLE KEYBOARD NAVIGATION
    ══════════════════════════════════════════════════════ */

    /* Enter on Amount → Debit of next row → open modal */
    window.addEventListener('keydown', function(e) {
        ...
        /* Amount → move to DrSlcd of same row */
        if (el.classList.contains('entry-amount')) {
            e.preventDefault(); e.stopPropagation(); e.stopImmediatePropagation();
            calculateTotal(); checkAddRow();
            var drslcd = el.closest('tr').querySelector('.dr-slcd');
            if (drslcd) { drslcd.focus(); drslcd.select(); }
            return;
        }

        /* DrSlcd → move to Debit of NEXT row, open modal */
        if (el.classList.contains('dr-slcd')) {
            e.preventDefault(); e.stopPropagation(); e.stopImmediatePropagation();
            var row = el.closest('tr');
            var nextRow = row.nextElementSibling;
            if (!nextRow) { addRow(); nextRow = document.querySelector('#entriesBody tr:last-child'); }
            selectRowEl(nextRow);
            selectedRow = nextRow;
            currentField = 'debit';
            setTimeout(function() { showAccountSelector(currentAccountType); }, 50);
            return;
        }

        /* Debit name → open debit modal */
        if (el.classList.contains('debit-name')) {
            e.preventDefault(); e.stopPropagation(); e.stopImmediatePropagation();
            var row = el.closest('tr');
            selectRowEl(row); selectedRow = row;
            currentField = 'debit';
            showAccountSelector(currentAccountType);
            return;
        }

        /* Credit name → open credit modal */
        if (el.classList.contains('credit-name')) {
            e.preventDefault(); e.stopPropagation(); e.stopImmediatePropagation();
            var row = el.closest('tr');
            selectRowEl(row); selectedRow = row;
            currentField = 'credit';
        ...
        /* Date → move to Debit, open modal */
        if (el.classList.contains('entry-date')) {
            e.preventDefault(); e.stopPropagation(); e.stopImmediatePropagation();
            var row = el.closest('tr');
            selectRowEl(row); selectedRow = row;
            currentField = 'debit';
            setTimeout(function() { showAccountSelector(currentAccountType); }, 30);
            return;
        }
    }, true);
    ...
    /* F-keys and Escape (already existing but now with modal check) */
    document.addEventListener('keydown', function(e) {
        if (_isModalOpen()) return; // Modal handles its own keys
...
function selectAccount() {
    ...
    /* After debit → open credit modal. After credit → focus amount. */
    if (currentField === 'debit') {
        currentField = 'credit';
        setTimeout(function() { showAccountSelector(currentAccountType); }, 80);
    } else if (currentField === 'credit') {
        setTimeout(function() {
            var amt = selectedRow.querySelector('.entry-amount');
