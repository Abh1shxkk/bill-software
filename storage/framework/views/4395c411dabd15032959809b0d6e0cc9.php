<?php $__env->startSection('title', 'Sale Return Replacement Transaction'); ?>
...
<div class="d-flex justify-content-between align-items-center mb-3">
    ...
    <div>
        <a href="<?php echo e(route('admin.sale-return-replacement.index')); ?>" class="btn btn-primary">
        ...
        <form id="saleReturnReplacementForm" method="POST" autocomplete="off" onsubmit="return false;">
            ...
            <div class="header-section">
                ...
                <div class="header-row">
                    ...
                    <div class="field-group">
                        <label>Date</label>
                        <input type="date" class="form-control" name="trn_date" id="trn_date" value="<?php echo e(date('Y-m-d')); ?>" style="width: 140px;" onchange="updateDayName()">
...
$('#saveBtn').click(function() {
    ...
    // ── Fire ──
    $.ajax({
        url: "<?php echo e(route('admin.sale-return-replacement.store')); ?>",
        method: 'POST',
...
<script>
// ============================================================
// AUTO-SAVE  —  sale_return_replacement_transaction_autosave_v1
// ============================================================
(function(){
'use strict';
const KEY = 'sale_return_replacement_transaction_autosave_v1';
let _t = null;
...
function save(){
    ...
    document.querySelectorAll('#itemsTableBody tr').forEach(function(tr){
        ...
        rows.push({
            ...
            batch_id:g('batch_id'), packing:g('packing'), company:g('company'),
            mrp:g('mrp'), unit:g('unit'), cl_qty:g('cl_qty'),
...
function restore(){
    ...
    _banner(state.savedAt, function keep(){
        ...
        if(typeof _rowCount!=='undefined') window._rowCount=0;

        (state.rows||[]).forEach(function(saved){
            const ri=saved.ri;
            if(typeof _rowCount!=='undefined'&&parseInt(ri)>=_rowCount) window._rowCount=parseInt(ri)+1;
            const tr=document.createElement('tr');
            tr.setAttribute('data-row',ri);
            tr.innerHTML=
                '<td><input type="text" class="form-control item-code readonly-field" name="items['+ri+'][item_code]" value="'+_esc(saved.item_code)+'" readonly>'+
                    '<input type="hidden" name="items['+ri+'][item_id]" value="'+_esc(saved.item_id)+'">'+
                '</td>'+
                '<td><input type="text" class="form-control item-name readonly-field" name="items['+ri+'][item_name]" value="'+_esc(saved.item_name)+'" readonly>'+
                    ...
                    '<input type="hidden" name="items['+ri+'][company]" value="'+_esc(saved.company)+'">'+
                    '<input type="hidden" name="items['+ri+'][mrp]" value="'+_esc(saved.mrp)+'">'+
                    '<input type="hidden" name="items['+ri+'][unit]" value="'+_esc(saved.unit)+'">'+
                    '<input type="hidden" name="items['+ri+'][cl_qty]" value="'+_esc(saved.cl_qty)+'">'+
                '</td>'+
                '<td><input type="text" class="form-control readonly-field" name="items['+ri+'][batch_no]" value="'+_esc(saved.batch_no)+'" readonly></td>'+
                '<td><input type="text" class="form-control readonly-field" value="'+_esc(_expiryDisp(saved.expiry_date))+'" readonly>'+
...
document.addEventListener('DOMContentLoaded',function(){
    setTimeout(function(){
        // Patch location.reload so draft clears on any success
    ...
    const form=document.getElementById('saleReturnReplacementForm');
...
<?php echo $__env->make('layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\bill-software\resources\views/admin/sale-return-replacement/transaction.blade.php ENDPATH**/ ?>