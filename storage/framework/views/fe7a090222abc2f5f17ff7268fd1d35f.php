<?php $__env->startSection('title', 'Breakage/Expiry to Supplier - Issued Transaction'); ?>
...
<div class="bsi-form">
    <div class="d-flex justify-content-between align-items-center mb-2">
        ...
        <a href="<?php echo e(route('admin.breakage-supplier.issued-index')); ?>" class="btn btn-outline-secondary btn-sm py-0"><i class="bi bi-list"></i> View All</a>
    </div>
        ...
        <div class="header-section">
            <div class="row g-2">
                <div class="col-md-2">
                    ...
                    <div class="field-group"><label style="width:40px;">Trn.No:</label><input type="text" id="trn_no" name="trn_no" class="form-control readonly-field" value="<?php echo e($trnNo); ?>" readonly style="width:60px;"></div>
                </div>
                <div class="col-md-10">
                                        ...
                                        <div id="supplierDropdown" class="custom-dropdown-menu">
                                            ...
                                            <div id="supplierList">
                                                <?php $__currentLoopData = $suppliers; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $s): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                                    <div class="dropdown-item" data-id="<?php echo e($s->supplier_id); ?>" data-name="<?php echo e($s->name); ?>">
                                                        <?php echo e($s->name); ?>
...
function saveTransaction() {
    ...
    fetch('<?php echo e(route("admin.breakage-supplier.store-issued")); ?>', {
        method: 'POST',
        body: formData,
    ...
    .then(data => {
        if (data.success) {
            ...
            window.location.href = '<?php echo e(route("admin.breakage-supplier.issued-index")); ?>';
        } else {
...
<script>
// ============================================================
// AUTO-SAVE ON REFRESH  —  bsi_issued_transaction_autosave_v1
// ============================================================
(function () {
    'use strict';
    const STORAGE_KEY = 'bsi_issued_transaction_autosave_v1';
    let _timer = null;
    ...
    function saveDraft() {
        ...
        const state = {
            ...
            note_type: _val('note_type'),
            noteTypeSearchInput: _val('noteTypeSearchInput'),
            tax_flag: _val('tax_flag'),
            inc_flag: _val('inc_flag'),
            gst_vno: _val('gst_vno'),
            rows: rows,
        };

        if (!rows.length && !state.supplier_id && !state.gst_vno) return;
        try { localStorage.setItem(STORAGE_KEY, JSON.stringify(state)); } catch(e) {}
        _badge();
    }

    function _schedule() { clearTimeout(_timer); _timer = setTimeout(saveDraft, 700); }
    ...
    function restoreDraft() {
        let state;
        try { const raw = localStorage.getItem(STORAGE_KEY); if (!raw) return; state = JSON.parse(raw); } catch(e) { return; }
        if (!state) return;
        if (!state.rows.length && !state.supplier_id && !state.gst_vno) return;

        _banner(state.savedAt, function keep() {
            if (state.transaction_date) {
                _set('transaction_date', state.transaction_date);
                if (typeof updateDayName === 'function') updateDayName();
            }
            _set('supplier_id', state.supplier_id || '');
            _set('supplierSearchInput', state.supplierSearchInput || '');
            _set('supplier_name', state.supplierSearchInput || '');
            _set('note_type', state.note_type || 'C');
            _set('noteTypeSearchInput', state.noteTypeSearchInput || 'C');
            _set('tax_flag', state.tax_flag || 'N');
            _set('inc_flag', state.inc_flag || 'N');
            _set('gst_vno', state.gst_vno || '');
    ...
    /* ---- clear ---- */
    window.clearAutoSave = function() { try { localStorage.removeItem(STORAGE_KEY); } catch(e) {} };
    ...
    /* ---- patch success (fetch interceptor) ---- */
    document.addEventListener('DOMContentLoaded', function() {
        setTimeout(function() {
            ...
            window.fetch = function(url) {
                ...
                if (s.indexOf('store-issued') !== -1 || s.indexOf('breakage-supplier') !== -1) {
                    return p.then(function(resp) {
                        resp.clone().json().then(function(d) {
                            if (d && d.success) clearAutoSave();
                        }).catch(function(){});
                        return resp;
                    });
                }
        ...
        const form = document.getElementById('bsiForm');
        if (form) {
            form.addEventListener('input', _schedule);
            form.addEventListener('change', _schedule);
        }
        const tbody = document.getElementById('itemsTableBody');
        if (tbody) new MutationObserver(_schedule).observe(tbody, { childList: true, subtree: true });
    });
})();
</script>

<?php $__env->stopPush(); ?>
<?php echo $__env->make('layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\bill-software\resources\views/admin/breakage-supplier/issued-transaction.blade.php ENDPATH**/ ?>