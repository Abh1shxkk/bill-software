...
<!-- ============================================ -->
<!-- DRAFT SYSTEM — Sale Return Transaction      -->
<!-- ============================================ -->
    ...
    <div class="draft-modal-body">
        <div class="mb-3 d-flex gap-2 align-items-center">
            <input type="text" id="draftSearchInput" class="form-control form-control-sm"
                placeholder="Search by customer, invoice no., or remarks..."
...
const DRAFT_STORAGE_KEY = 'sale_return_transaction_drafts';
...
function collectFormState() {
    ...
    // Collect each row — use row id="row-N" pattern
    const items = [];
    document.querySelectorAll('#itemsTableBody tr').forEach(row => {
        const rowId  = row.id; // "row-N"
        const idx    = rowId ? rowId.replace('row-', '') : null;
        if (idx === null) return;
...
function saveDraft() {
    ...
    if (!hasCustomer && !hasInvoice && !hasItems) {
        showAlert('warning', 'Nothing to save as draft. Please select a customer or add items first.');
        return;
    }

    const custLabel    = header.customer_text
        ? header.customer_text.split(' - ').pop().trim()
        : (header.customer_id ? 'Customer #' + header.customer_id : 'Unknown Customer');
    const invoiceLabel = hasInvoice ? 'Inv#' + header.original_invoice_no : '';
    const draftName    = [invoiceLabel, custLabel].filter(Boolean).join(' — ');
...
function loadDraftById(draftId) {
    ...
    if (currentRows > 0 || currentCust) {
        // Use native confirm since this file doesn't use showConfirm pattern
...
function applyDraftToForm(draft) {
    ...
    const tbody = document.getElementById('itemsTableBody');
    tbody.innerHTML = '';
    // Reset global row counter so IDs are consistent
    currentRowIndex = 0;

    (items || []).forEach((itemData, _i) => {
        // Rebuild the item object that addItemRow() expects
...
// Ctrl+D = Save Draft (when no modal open)
...
console.log('✅ Draft System Loaded — Sale Return Transaction');
</script>
