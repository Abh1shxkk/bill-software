...
function saveReceipt() {
    ...
    if (currentReceiptId) {
        ...
        url = `<?php echo e(url('admin/customer-receipt')); ?>/${currentReceiptId}`;
        method = 'PUT';
    } else {
        // Create new receipt
        url = '<?php echo e(route("admin.customer-receipt.store")); ?>';
        method = 'POST';
    }
...
function openAdjustmentModalForRow(row) {
    ...
    let url = `<?php echo e(url('admin/customer-receipt/customer-outstanding')); ?>/${customerId}?page=1&per_page=100`;
    if (currentReceiptId) {
...
function loadPastInvoices() {
    ...
    fetch(`<?php echo e(url('admin/customer-receipt/get-receipts')); ?>`, {
        method: 'GET',
        headers: {
...
function loadReceiptById(id) {
    fetch(`<?php echo e(url('admin/customer-receipt/details')); ?>/${id}`)
        .then(r => r.json())
        .then(data => {
            if (data.success) {
...
<?php echo $__env->make('layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\bill-software\resources\views/admin/customer-receipt/modification.blade.php ENDPATH**/ ?>